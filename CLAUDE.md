...
**RBAC** — Always query the `user_permissions` view (not the raw relation tables). It aggregates roles → permissions per user but returns **every grant row** — `(user_id, action, permission_code)` is *not* unique, so a user with two roles gets a row from each. Effective access is the **union** of those rows, resolved by `packages/authz-core` `decide()`; never treat one row as the answer. For *display* (SY01 実効権限) the rows are folded to **one row per permission code** by `groupPermissionsByCode()` in the same package (widest scope per `(code, action)` via `highestScopeRows()`, then grouped by code with the actions listed inside; an `ADMIN` grant hides the actions it subsumes) — presentation only, never for authorization. For *inspection* (「誰が何を持っているか」を一覧で読む — Metabase / psql / `tools/rbac-matrix`) there is the DB-side twin `app.user_permission_summary` (one row per user: `roles`, `permission_codes`, `grants` as `code:ACTION@SCOPE[codes]`, `permissions` jsonb, `is_superuser`; Metabase reads it as `analytics.v_user_permissions`) — same rule: read it, never decide with it. Roles and grants are owned by two idempotent seeds — `shared-db/sql/rbac-seed.sql` (the permission codes + `admin`/`staff`) and `roles-seed.sql` (the 15 operational roles) — **not** by the app. `system` and `kiosk`, plus the five **privileged codes** (`kiosk_secret` / `kiosk_device` / `kiosk_card` / `personal_data` / `user_admin`), are excluded from every business role. **When you add a permission code, check both seeds' exclusion lists**: `roles-seed.sql` grants `manager` / `viewer` / the six `*_manager` roles via `CROSS JOIN app.permissions`, so a new code is handed to all of them unless you exclude it — this already happened once with `kiosk`. `rbac-seed.sql` also deletes every `APPROVE` grant, excluding only the five privileged codes; widening that DELETE empties `privileged_approver` and silently makes every request undecidable. Two derived references are regenerated, never hand-edited: `_docs/rbac-role-matrix.xlsx` (`tools/rbac-matrix/build_rbac_xlsx.py`, reads the live DB + `app-list.ts`) and the DC02 internal doc「ロールと権限」(`content/internal/rbac/`). Launcher visibility is a separate axis — `feature_flags` (`feature-flags-seed.sql`) decides what is published on `main`, so a grant never publishes an app.
...
- `POST /extract/<doc_type>` (multipart: `file` + optional `prompt`) — uses a **built-in schema** per type. Types: `order-request` (注文請書 intake — the primary one), `purchase-order` + `material-delivery` (the 購買 pair, below), `quote`, `invoice`, `delivery-note`. Each schema matches the v3 data model. **Every prompt states the direction first** — which side of the paper is us and which is the other party — because that is the mistake that fails silently: get it backwards and our own name lands in `customer_name` / `supplier_name`, where it can never match anything.
...
**購買側の取込（PU02 / PU03）** — the same pipeline read from the other end: the documents a **supplier sends us**. Two doc types, two entry points, and **no numbering and no DB row until the user saves** — unlike the sales side (which mints an `IMPORT` 注文請書 before extracting), these read a file and fill a form, so a failed read costs nothing and needs no retry queue.
- **素材発注書 (PU02)** — `/purchase/purchase-orders/new` gets an「AI で読み取る」panel: a supplier's 見積書 / 注文請書 / 発注書控え → `POST /api/extract/material-order` (gate `purchase_order:CREATE`) → `/extract/purchase-order` → supplier + per-line material matched → a review step → 「フォームに反映」 fills the existing `PurchaseOrderForm`. The form's own validation and submit path are untouched.
- **素材入荷 (PU03)** — a 納品書 has many lines but `material_receipts` is **one row per material**, so it gets its own page `/purchase/material-receipts/intake` (linked from the PU03 list, gated `material_receipt:CREATE`): `POST /api/extract/material-delivery` → `/extract/material-delivery` → an editable table with per-line「この行を登録しない」→ one Server Action creates every selected line **in a single transaction**, posting inventory through the same `onMaterialReceipt` the 1-件 form uses. Half-registered deliveries are the one outcome worth a transaction here.
- Matching mirrors the sales side but swaps both ends: the BP pool is loaded with the **VENDOR** role (`loadBpMatchPool(role)` — the role only decides *auto-confirm*, never who is in the pool), and materials are resolved by `lib/material-match.ts`. **The material code wins outright** (`B01A0001-A060-310` is printed straight back at us by suppliers); names fall through to the same staged `text-match` tiers as products, sharing `productMatchKey` so dimension notation (`φ8.3×330` / `8.3x330`) collapses the same way. The pool is read whole — materials number in the thousands, not the tens of thousands, so no probe ladder.
- **Nothing is invented.** A line that does not resolve keeps its printed text on screen and stays empty; PU02 lets it through to the form (whose validation stops the save) and PU03 refuses to register it. What the user picks is then learned into `match_aliases` (`target_type` gains **`materials`**, keyed by `productMatchKey`), so the next document in the same supplier's format resolves itself.
- The source PDF/image is attached to what gets created (`material_purchase_orders` / `material_receipts`) — **best-effort: a failed attachment never rolls back the document**.
- Extraction failures go through `lib/intake-extract-error.ts`, so the user sees cause + next step, and a key/model mistake says plainly that retrying will not help. Logic split: `lib/purchase-intake-core.ts` (pure normalisation + tests) / `lib/purchase-intake.ts` (server: po-extract call, matching, alias learning).

**再研磨（REGRIND — customer tools reground, not manufactured）** — a 注文請書 line with 注文種別 `REGRIND` becomes a `WORK_ORDER_TYPE.REGRIND` 指示書 on the same 注文明細 → 指示書 → 出荷書 → 請求書 chain (no separate app — PD02 filters by 種別). **What is sold is the service, what is held is the tool** — see the 再研磨 pricing paragraph above: the line's `item_id` is a `REGRIND` item (MS0H, where the price is) and `tool_item_id` is the tool. Other makers' tools are **external products** (`items.is_external_product` + `maker_name`) and are **never sold** — no price, no price list, tool field only; no manufacturing route, no 製造/在庫 指示書 (guards: picker `includeExternal`, `lib/sales-item-guard.ts`, readiness `externalProduct` / `regrindTool`, `validateExternalProductType`). **The customer's tools never become our stock**: `item_inventory.owner_bp_id` is a second axis next to `custody_bp_id` (custody = who holds it, owner = whose it is; both may be set when a customer's tool is at a coater), and every own-stock query must say `ownerBpId: null` — `inventory-custody-scope.test.ts` scans both axes. Ledger: the start step `REGRIND_RECEIPT`（製品受入（再研磨）） books 受入本数 into the owner bucket on completion (cause `REGRIND_RECEIPT`, stamp `work_order_steps.regrind_receipt_movement_id`, same tx in web `lib/workflow.ts` and kiosk `step-execution.ts`; rollback refused afterwards); WO completion posts **no** finished/semi IN and no material consumption, only OUTs 返却本数 (= the receipt step's 廃棄 bucket, relabelled 返却（再研磨不可）; `regrindQuantities()`); DISPATCH ships from the owner bucket (`ownerBucketFor`, `lib/inventory-owner-core.ts` twin); outsource issue/return move owner bucket ⇄ owner+custody bucket. Steps come from a **common 再研磨工程リスト** (`product_process_routes.kind = REGRIND`, PREP-shaped, `/master/process-steps/regrind-routes`, always latest version via `applyLatestCommonRoute`; edits on the WO drop the route link instead of minting versions). Which steps a WO type may hold is **master data** — `process_step_catalog.allowed_work_order_types` (MS08, a non-empty multi-select of 在庫分 / 製造分 / 再研磨), read through `stepAllowedForType` / `typeCompositionIssues` in `lib/workflow-core.ts`; server validation, WorkflowBuilder and RouteEditorForm all go through that one function. It used to be guessed from the process category, which could not express a shop's exceptions (re-cutting a cylinder during a regrind, receiving-inspecting a from-stock lot) without a code change. **The two start steps stay pinned in code** (`pinnedWorkOrderType`): PRODUCT_ISSUE is FROM_STOCK-only and REGRIND_RECEIPT is REGRIND-only, because consuming reserved stock and booking the owner bucket both depend on the WO's type — the master value is not even read for them. Allocation invariants (`work-order-alloc-core.ts`): a REGRIND WO takes exactly one REGRIND line, planned = allocated, and a completed REGRIND WO counts as fully served even when tools were returned. 受入本数 is the one place the client's value is authoritative (`resolveReceivedQuantity({ clientAuthoritative })`) — the tools that arrive are not the tools that were promised. The defect type `REGRIND_RETURN` is demo/master data (`extended-master-seed.sql`), not a migration.
...
**Privileged access (特権アクセス, SY0G)** — System-critical operations are no longer "you hold the grant, so you can do it". The coarse `kiosk` / `system` codes were split into five privileged codes, and doing the work needs a **second person's approval, bounded in time**. Two mechanisms, because the operations have two shapes:
- **A. 時限昇格** (`privileged_access_requests` + `_operations`) — for operations whose target can't be named in advance (reveal a PIN, print cards, search history). One request = one code + N chosen operations; the approver may **untick** some before approving (`granted`). **The clock starts on first use, not on approval**, and the grant dies at `min(first use + duration, window end)` — shorter wins. A request may not reach beyond **14 days** from the request (validator + DB CHECK). Codes: `kiosk_secret`, `kiosk_device`, `kiosk_card`, `personal_data`.
- **B. 変更依頼** (`user_change_requests`) — for operations that *are* one concrete change (suspend this person, change their plants). Approval **applies** the change, by re-running the normal mutation so a moved premise lands as `apply_error` instead of being forced. Code: `user_admin`. Same shape as `work_order_flow_changes`.

Enforcement is `lib/privileged-access.ts`, layered **after** `decide()` (authz-core is unchanged). **`peekElevation` vs `useElevation` is the rule that matters**: rendering a screen must only peek, or merely opening SY09 would start someone's clock; only the action that actually performs the operation calls `useElevation`, which arms and checks in one conditional UPDATE (no check-then-use gap, and already-running grants are spent before fresh ones). **Expiry is never a cron job** — `user-suspension-cron.sql` can lag a minute because lag there means *less* access; here it would mean *more*, so `status = 'EXPIRED'` is a display stamp only. `system:ADMIN` is deliberately **not** gated (the user's call) — it is also the only anti-lockout path, which is why no self-approval exists; the bypass is recorded as `bypass: "admin"` in the audit row. The operation registry (23 entries) is `lib/privileged-operations.ts`: adding a privileged operation = one entry there + one `useElevation` line at the call site. Per-document 履歴 tabs are deliberately **not** gated — only SY07's cross-document search — because an approval in front of every 見積書 detail page is how the control gets switched off.
...
**Secrets (never commit)** — The **Cloudflare DNS API token** (acme.sh DNS-01 for `nginx-proxy`; `Zone:DNS:Edit` on `kai-lab.net` + `ckk-tool.co.jp`) has its operational copy in the server's `~/stacks/nginx-proxy/.env` (`CLOUDFLARE_DNS_API_TOKEN`). A local backup lives in this Mac's login Keychain — retrieve with `security find-generic-password -s ckk-cloudflare-dns-api-token -w`. The **Cloudflare Tunnel API token** (account-scoped, `Cloudflare Tunnel:Edit` — used to manage the tunnel's public-hostname ingress rules via API, e.g. adding `deploy.ckk-tool.co.jp`) lives only in the Keychain: `security find-generic-password -s ckk-cloudflare-tunnel-api-token -w`. Tunnel config API: `PUT /accounts/f3ed926bb74cda704944f32bea936b5e/cfd_tunnel/3c8475a0-8285-4f44-a8d2-b1e0efb50c5b/configurations` (GET first, edit the `ingress` array, PUT back whole). If either token is exposed, rotate it in Cloudflare and update its storage place(s). **`CRED_ENCRYPTION_KEY`** (Fernet) encrypts the admin logins that adminTools' 管理者ログイン modals save (Sakura control panel / King of Time); the operational copy is `/data/coolify/source/.cred-encryption-key` on the server, it is set as an env var on **three** Coolify apps that must all hold the *same* value (`admintools-dev` / `admintools-main` / `kot-import` — kot-import decrypts what admintools-main wrote), it is provisioned idempotently by `coolify/platform/add-cred-encryption-key.sh`, and the Keychain backup is `security find-generic-password -s ckk-cred-encryption-key -w`. **Do not rotate it casually** — a new key doesn't lock anyone out but silently orphans every credential saved through those modals (they fall back to the `SAKURA_ID`/`SAKURA_PW`/`KOT_ID`/`KOT_PW` env vars).
