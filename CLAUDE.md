...
The history was **squashed on 2026-08-24** into 6 readable baseline migrations (`20260824000001`–`000006`: schemas/enums → master tables → business tables → system tables → constraints+indexes+FKs → views/functions/triggers). FKs live in the constraints file so table-file membership never matters. Data that used to sit inside migrations (材種・素材 from Excel, 工程マスタ, 採番マスタ, 承認フロー, 通貨, `system` user) now lives in `shared-db/sql/baseline-seed.sql`, applied **once** on a new DB. `prisma migrate diff` against a baseline-built DB must be **empty** — GIN `match_names` indexes and the `settings_code` default are declared in the schema now, so any diff is a real change.

**Migrations apply themselves.** Merging to `dev` / `main` triggers the Coolify apps `db-migrate-dev` / `db-migrate-main` (watch path `shared-db/**`), which run `prisma migrate deploy` → `grants.sql` → `kiosk-cron.sql` → `analytics-views.sql` against that environment's DB and fail the deployment if any step fails. Do not apply migrations by hand as part of normal work.
