...
対象: `origin/dev` @ `40ed2c1e`（#897 merge、main から 18 PR 先）。検証後に #898 / #899（第 3 段）が dev に入ったので、そこへ merge し直して全検査を再実行した。
...
4. **シード**: デモシードの行に品目 id が無く、素材名が空・工程リストが「未登録」に見える — **#899（品目統合 第 3 段）が旧列ごと落としてシードを品目 id で書き直したため、このブランチの後付け（`demo-items-backfill.sql`）は取り下げた**
...
11. **品目**: 指示書が旧 `material_id` を書かない（#896 の方針違反）— **#899 で旧列そのものが消えたので取り下げ**
