...
| 種別 | 件数 | 内容 | 状態 |
|---|---|---|---|
| `pageerror` React #418 | 18/18 | **全画面**（未登録端末の `/setup` `/login` も）。`next dev` では出ない。原因は **`NEXT_PUBLIC_APP_VERSION` のビルド時と実行時のずれ**: `NEXT_PUBLIC_*` はクライアント束にビルド時へ焼き込まれ、サーバー描画は実行時 env を読むので、両者が違うとフッターの版表示が食い違う。ビルドと起動に同じ値を渡すと消える（web 側でも同じ条件で再現し、揃えると消えることを確認した = 原因の確定）。**実機では起きていない** — `app-dev` のフッターは `v0.1.0`・ページエラー無し（2026-09-10 確認）。ただし Coolify の実行時 env だけ上げると壊れる潜在的な罠なので `coolify/platform/README.md` に注意を書いた | 対象外（アプリの欠陥ではない） |
...
- 全画面のハイドレーション不一致（React #418）は**アプリの欠陥ではなく `NEXT_PUBLIC_APP_VERSION` のビルド時 / 実行時のずれ**（§3.2）。同じ条件で web 側にも再現し、値を揃えると両アプリとも消えることを確認したので原因は確定。実機（`app-dev`）では起きていないが、**Coolify の実行時 env だけ版を上げると全画面がこうなる**ため `coolify/platform/README.md` に注意書きを足した。
...
**依存の更新後（next 16.3.4 / @tiptap 3.31.3 / nodemailer 9.1.1）に、まっさらな種から
もう一度**（2026-09-10）:

| 検査 | 結果 |
|---|---|
| web 巡回（260 画面 × 2 幅） | 486 件・**pageerror ゼロ**（横スクロール 2 = `/preview` と価格表の新規、残りは a11y の既知分） |
| 共有端末 巡回（18 画面 × 2 向き） | 26 件・**pageerror ゼロ**（5 タップの隠しトリガー 16 / 推測 URL の 404 / 誤検知） |
| 共有端末 検査承認 | 12 / 12 PASS |
| 出荷 + 最終検査 | 19 / 19 PASS |
| smoke-flows | 92 PASS |

**通し確認 3 本はいずれも「1 回の種に 1 回だけ」走らせる。** 同じ DB で 2 回目を走らせると
状態が残って落ちる — 検査承認は 1 回目で承認を消費し、出荷は工程を進め、smoke-flows は
列設定を保存する（前回点検が smoke-flows について書いた state 依存は、実は 3 本ともに効く）。
落ちたら**まず `docker rm -f ckk-shots-db` から種を作り直す**こと。
