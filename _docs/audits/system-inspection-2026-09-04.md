...
| #827 | 基盤・認証の残り（gateway の `(uid, sha256)` 重複排除・定期処理 3 本の advisory lock・ログインの IP 単位レート制限を DB へ・`setup/confirm` にリンクコード所持を要求・SSO 自動プロビジョンのローカル行保護） | 3.6 M / 4.2 |
...
**未対応（改善計画 §4 に残す）**: なし — **この点検で挙げた指摘は #804–#827 で全部
片付いた**。最後まで残っていたぶんの行き先は、出荷が他明細の予約を無視（3.4 F8）・
税区分と税率の書類スナップショット・受取先言語 PDF の明細ラベル・円未満の丸め方針が
**#825**、購買の部分入荷 UI と短納クローズが **#826**、`setup/confirm` のリンクコード要求・
gateway の `(uid, sha256)` 重複排除・定期処理 3 本の advisory lock・SSO 自動プロビジョンの
ローカル行保護・ログインの IP 単位レート制限が **#827**。

ただし**改善計画 §4 が空になったわけではない**。§4.2 のうち指摘に紐づかない整備
（特権操作の監査注記の必須化・`roles-seed.sql` の役割を決める）と、§4.3 の構造的な
改善は手つかずのまま残っている — あちらは個別の不具合ではなく、次の四半期の作業として
読むこと。
...
#827: (1) `setup/confirm` は**共有端末アプリを丸ごとデプロイしてから**効く
（サーバーだけ先に出ると、古い画面が有効化の瞬間に PROOF_REQUIRED で止まる —
その場合は端末で `/setup` を再読み込みしてリンクからやり直す）。
(2) **`app.users` にパスワードを持つ行は SSO で入れなくなる**。初期管理者
`admin` が該当するので、IdP に同名アカウントがあるなら先に確認すること
（`SELECT username FROM app.users WHERE password_hash IS NOT NULL`）。
(3) advisory lock は専用の接続を 1 本張る（Prisma のプールではロックを取った
接続と解放する接続が別になり得るため）。ティック中だけなので常時接続は増えない。
