...
| #824 | 基盤・認証の残り（gateway の `(uid, sha256)` 重複排除・定期処理 3 本の advisory lock・ログインの IP 単位レート制限を DB へ・`setup/confirm` にリンクコード所持を要求・SSO 自動プロビジョンのローカル行保護） | 3.6 M / 4.2 |
...
**未対応（改善計画 §4 に残す）**: 出荷が他明細の予約を無視（3.4 F8）、税区分・税率の書類スナップショット（スキーマ変更が要る）、受取先言語 PDF の明細ラベル、円未満の丸め方針、購買の部分入荷 UI と短納クローズ。
（#824 で、`setup/confirm` のリンクコード要求・gateway の `(uid, sha256)` 重複排除・
定期処理 3 本の advisory lock・SSO 自動プロビジョン・IP 単位レート制限が片付いた。）
...
#824: (1) `setup/confirm` は**共有端末アプリを丸ごとデプロイしてから**効く
