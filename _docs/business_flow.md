...
| § | ID | ノード | 担当 | 主な文書 | 内容 |
|---|-----|--------|------|----------|------|
| §1 | P0 | 価格試算 | 社内営業 | 価格試算 | 工具種別の原価計算で見積単価を算出（EST-）。任意で製品にリンク（1製品に複数価格試算可）。確定で価格ソースになる |
| §1 | P1 | 価格表作成 | 社内営業 | 価格表、価格試算 | 顧客＋製品＝1価格表。注文種別ごとに基準単価＋本数範囲（倍率）＝単価、有効日を管理。製品にリンク済みの確定価格試算を基準単価ソースに選択可（手動設定も可） |
...
label: 価格表;
side: right;
title: 顧客＋製品＝1価格表。注文種別ごとに基準単価＋本数範囲（倍率）＝単価。有効日は種別単位;
...
[role_sales: 価格試算を作成・確定] <block_apply>
label: 価格試算;
desc: 工具種別の原価計算で見積単価を算出（EST-YYYYMM-NNNNN）。任意で製品にリンク（1製品に複数価格試算可）。確定した価格試算が価格表の基準単価ソースになる。;
[role_sales: 価格表を作成・改訂] <block_apply>
label: 価格表作成;
props: PRICE_LIST;
desc: 顧客＋製品コードで1価格表。注文種別（本番/テスト/サンプル/その他）ごとに基準単価＋本数範囲（倍率）＝単価、有効日を設定。製品にリンク済みの確定価格試算を基準単価ソースに選択できる（手動設定も可・使用時に価格試算はロック）。;
...
[role_system: 会計ソフトへ連携（仕訳 CSV エクスポート）] <block_system>
label: 会計連携;
desc: 生成した請求データを会計ソフト（TKC FX4クラウド）へ仕訳 CSV でエクスポート。lib/accounting-export-core.ts が生成し、列の並びと文字コードは SY0J 会計連携が持つ。;
