...
| § | ID | ノード | 担当 | 主な文書 | 内容 |
|---|-----|--------|------|----------|------|
| §1 | P0 | 試算 | 社内営業 | 試算 | 工具種別の原価計算で見積単価を算出（EST-）。任意で製品にリンク（1製品に複数試算可）。確定で価格ソースになる |
| §1 | P1 | 価格表作成 | 社内営業 | 価格表、試算 | 顧客＋製品＝1価格表。注文種別ごとに基準単価＋本数範囲（倍率）＝単価、有効日を管理。製品にリンク済みの確定試算を基準単価ソースに選択可（手動設定も可） |
...
[role_sales: 試算を作成・確定] <block_apply>
label: 試算;
desc: 工具種別の原価計算で見積単価を算出（EST-YYYYMM-NNNNN）。任意で製品にリンク（1製品に複数試算可）。確定した試算が価格表の基準単価ソースになる。;
[role_sales: 価格表を作成・改訂] <block_apply>
label: 価格表作成;
props: PRICE_LIST;
desc: 顧客＋製品コードで1価格表。注文種別（本番/テスト/サンプル/その他）ごとに基準単価＋本数範囲（倍率）＝単価、有効日を設定。製品にリンク済みの確定試算を基準単価ソースに選択できる（手動設定も可・使用時に試算はロック）。;
