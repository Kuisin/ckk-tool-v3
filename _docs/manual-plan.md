...
| slug | 内容 | 主な題材 |
|---|---|---|
| `flow/sales` | 販売フロー: 試算 SA01 → 価格表 SA02 → 見積書 SA03 → 受注請書 SA04 → 注文請書 PD01 | 見積単価の自動解決（価格表 tiers）、試算スナップショット（確定後は再計算されない）、AI 取込（PDF→自動入力）、価格差異 (PRICE_DIFF) の再調整、伝票展開 |
...
### 付随して見つかった不具合（マニュアルとは別に修正が必要）

1. **納品書の検証メッセージが製品名でなく内部 ID を出す** — `validateItemsAgainstShipment` の
   `製品 ${productId} は出荷書に含まれていません`。コメントは「製品名入り」と書いてあるが実際は数値 id。
2. **請求書の税ラベルが 10% 固定** — 表示は「消費税（10%）」だが金額は顧客ごとの税率（10/8/0%）で計算される。
3. **製品フォームの「製品コード」が内部 ID を表示** — `ProductForm.tsx` の disabled 入力が
   `initial?.id`（数値）を出しており、`PRD-` コードではない。複製モーダルの「複製元」も同様。
4. **一部の設定画面にアクション/スコープの日本語ラベルが無い** — SY01 実効権限テーブルは
   `READ` / `ALL` などの英字をそのまま表示している。マニュアルでは読み方を補足したが、
   UI 側で日本語化するのが本筋。
