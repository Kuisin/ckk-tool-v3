...
| slug | 内容 | 主な題材 |
|---|---|---|
| `flow/sales` | 販売フロー: 価格試算 SA01 → 価格表 SA02 → 見積書 SA03 → 注文請書 SA04 → 注文請書 PD01 | 見積単価の自動解決（価格表 tiers）、価格試算スナップショット（確定後は再計算されない）、AI 取込（PDF→自動入力）、価格差異 (PRICE_DIFF) の再調整、伝票展開 |
...
| `flow/purchasing` | 購買フロー: 購買依頼 PU01 → 承認 → 素材発注書 PU02 → 素材入荷 PU03 → 素材在庫。付録: 外注依頼 PU04 | 承認フロー（REQUESTED→APPROVED→ORDERED→COMPLETED）、入荷予定と在庫予約、直接調達入荷、仕入実績が価格試算の参照単価になる連環 |
| `flow/shipping-billing` | 出荷・請求フロー: 出荷書 SH01 → 納品書 SH02 → 請求書 BL01 → 締日処理 BL02 → 会計エクスポート | 在庫引落し、直送/通常納品の違い（価格記載の有無）、締日と請求期間、会計連携 CSV |
...
| `admin/app-settings` | アプリ設定の全体像: SY02 価格試算計算（計算基準・工具種・カスタム入力・後処理フック — 変更しても確定済み価格試算は再計算されない）、SY03 製品項目 / SY04 製品種別、system_settings ストアの考え方（設定は即時反映・履歴は操作履歴 SY07 で追える） |
...
- キオスク運用（カード・端末・設定）は既存の `system/kiosk-*` 3 ページが
  ...
  引き続き `/admin-manual`（社内）。

## 3. 既存ページへの接続（小修正）

- `start.md` §3 操作コード例とアプリ紹介順を再編後のコード（SA01 価格試算…）の
