...
**土台は実装済み。業務の書き込み口はまだ開いていない**（下の「なぜ」を参照）。
...
1. **ルートハンドラは `prisma.x.create` を呼ばない。** Server Action が呼ぶのと
   ...
   **`scripts/check-api-gates.mjs` が機械で止める** — 書き込みメソッドの本文に
   `prisma.*.create/update/upsert/delete` が現れたら CI が落ちる。
2. **`Idempotency-Key` は必須**（任意ではない）。採番は
   `allocateDocumentKey()` の月次リセット連番なので、webhook の再送で `ORD-` が
   2 本立つと取り返しがつかない。鍵・要求の指紋・応答を
   `app.api_idempotency_keys` に保存し、同じ鍵の再送には**保存した応答を
   そのまま返す**（再実行しない）。同じ鍵で違う本文は **422**。
   窓は 24 時間。**成功した書き込みだけ覚える** — 失敗を覚えると、直しても
   同じ誤りが永久に返る。
3. **`If-Match` は必須 → 一致で実行 / 不一致で 409 / 無指定で 428。**
   無指定を素通しさせない — 「付ければ安全、付けなければ last-write-wins」では
   いちばん雑な呼び出し側がいちばん危険な経路を通る。意図した上書きは
   `If-Match: *` と明示させる（明示は記録に残る）。ETag は `updated_at` から
   作る弱い ETag なので、差分同期の順序キーと同じ値になる。
4. 承認・特権操作は口を作らない（§3.2）。

実装: `lib/api-write-core.ts`（純ロジック・試験あり）と `lib/api-write.ts` の
`runWrite()`。書き込みの口はこの形以外を書かない。

### 8.2 なぜまだ開いていないのか

候補になる書き込みは、いまどれも**業務規則が Server Action の中に直書き**
されている（`createManualAcceptance` は検証・価格表の解決・参照の整合・採番・
営業担当の決定・監査を 1 つの関数に持つ）。そのまま API から呼べないので、
口を開けるには先に `lib/` への抽出が要る。抽出せずに書けば規則が二重になり、
**それはこの API がいちばん避けたいこと**（§1）。

つまり残っているのは API 側の仕事ではなく、**業務ロジックの置き場所を直す
仕事**。読み取りの口はこれに依存しないので、先に運用できる。
