...
- **通貨**: 見積は 1 ドキュメント 1 通貨。デフォルトは顧客の取引通貨（`bp_customer_attrs.currency`）。
  価格表解決は見積通貨と一致する `price_lists` 行のみ対象（通貨不一致行は解決対象外として警告）
- **拠点**: 見積には起票拠点（`org_unit_id`）を必須付与。RBAC スコープの可視範囲はこれで判定
...
- 通貨は見積から継承（注文受諾書以降の通貨変更は不可。変更が必要な場合は見積からやり直す）
