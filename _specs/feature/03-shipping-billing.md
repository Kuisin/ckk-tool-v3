...
- 在庫保管分は請求フロー（§9）の対象外
- 出荷先・配送方法・エンドユーザーは**注文明細ごと**に持つ（§2 — 1 通の
  注文書の中で行ごとに届け先が違う注文があるため、ヘッダではなく行が
  唯一の持ち主）。1 出荷書に束ねられるのは同一顧客 × 同一出荷先 ×
  同一配送方法（ユーザー直送はさらに同一エンドユーザー）の明細だけ —
  出荷先・配送方法が違うと物理的に 1 つの荷物にならないため
  （`combinabilityError`）
