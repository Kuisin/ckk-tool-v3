...
**多言語化する前に、語を決めるための文書。** 画面の文言をひとつずつ訳し始める
前に、繰り返し出てくる語（書類名・状態・操作・工程用語）の訳をここで固定する。
訳語が画面ごとに揺れると、同じ「差し戻し」が Sent back と Rejected と Returned
に割れ、利用者から見て別の操作に見える。

- **この表が i18n の正**。`messages/{ja,en,zh}.json` を書くときは必ずここを引く。
- 表の 3 列（ja / en / zh）は **1 対 1** で固定する。同じ ja に 2 つの訳を当てない
  （文脈で割れる語は §5「要確認」に上げ、割る根拠と一緒に決める）。
- **確認のしかた** — 各表の「要確認」欄が空でない行だけ見れば足りる。空欄は
...
- **Sentence case**（`Work order`）。Title Case にしない。固有名詞と略語は大文字のまま（PDF / QR / AI / CSV）。
- 画面ラベルは**名詞**、ボタンは**動詞の原形**（`Save` / `Approve` / `Send back`）。
- 帳票名は単数（`Quote`, `Invoice`）。一覧のタイトルも単数扱いにせず複数（`Quotes`）— 一覧＝集合だから。
- 数量は `pcs`（本・個）。キオスクの既訳に合わせる。
- 「〜しました」は過去形の完了文（`Saved`, `Deleted`）。主語を足さない。

### 2.2 中国語 (zh)

- **簡体字・大陸の製造業慣用**。キオスクの既訳（工单 / 工序 / 良品 / 报废 / 半成品）を正とし、Web もそれに揃える。
- 状態は「已＋動詞」（已批准 / 已出货 / 已取消）、待ち状態は「待＋動詞」（待审批 / 待启用）で統一。
- 「〜書」は文書としての語尾を訳し分ける（単 / 书 / 表）。日本語の字面をそのまま持ち込まない（例: 請求書 ≠ 请求书）。
- 全角括弧・句読点は中国語のもの（，。（））を使う。
