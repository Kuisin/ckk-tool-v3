...
| 11 | **DB の多言語データは可変キー JSON + モーダル編集**（§2.10）。言語ごとの入力欄を並べない | 全マスタ画面 / `_specs/tables.md` の `{ ja, en }` 規約 |
...
4. 決定 11 の実装 — `{ ja, en }` → 可変キー JSON、マスタフォームを「名称 + 多言語モーダル」に、`_specs/tables.md` の規約更新。
