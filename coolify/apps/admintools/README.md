...
3. **バックアップ / 復元**（`/backup`）— shared-db（`ckk`）と SeaweedFS ストレージ、
   ...
   同じトークン）。詳細・エージェント側の設定は `coolify/common/db-backup/README.md`。
...
rollback), both `build_pack: dockerfile`, `base_directory: /coolify/apps/admintools`:
...
- **Watch paths:** both apps have Coolify `watch_paths =
  coolify/apps/admintools/**` — GitHub push webhooks redeploy them only when
  admintools files changed (see `../../README.md`).
- **Deploy / rollback:** `coolify/platform/deploy.sh admin-dev [<git-sha>]`
