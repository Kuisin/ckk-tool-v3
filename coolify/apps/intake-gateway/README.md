...
フラグは**書き込みの後**に打つので、その 2 つの間で IMAP が切れると次の巡回で
同じメールがまた未読で降ってくる（順序を入れ替えても、今度は書き損ねが黙って
消えるだけで穴が移動する）。そこで添付単位の冪等キー **`(uid, 添付の sha256)`**
を `INTAKE_DIR/.gateway/seen.json` に残し（`gateway/seen.py`）、一度置いた添付は
二度置かない。

- **キーに uid とハッシュの両方を使う。** uid だけでは 1 通の複数添付を
  区別できず（前回一部だけ書けたかもしれない）、ハッシュだけでは同じ注文書を
  客が本当に 2 通送ってきたときに 2 通目を捨ててしまう。
- 記録するのは**置けた添付だけ**。失敗した添付は覚えないので、次に届いたら
  また試す。
- 保持は **90 日**。読み込みのたびに古い記録を刈るので無限には伸びない。
- 台帳が壊れていたら**捨てて作り直す**。読めないことを理由に取込を止めると
  注文書が届かなくなる — 重複のほうが軽い（人が消せる）。
- ドットディレクトリなので nextjs-web のポーラーからは不可視。取込フォルダごと
  作り直したときは台帳も一緒に消えるので、状態が食い違わない。
