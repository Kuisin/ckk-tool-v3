...
24. 実行画面の戻るボタンが「指示書へ」になり、完了後も指示書ビューへ戻る。
    下書き/承認依頼中/完了/キャンセルの指示書では警告が出て工程を開けない

## デプロイ

Coolify（`nextjs-kiosk-dev` :3006 / `nextjs-kiosk-main` :3007、
`coolify/platform/README.md`）。公開: `ckk-kiosk-dev.kai-lab.net` /
...
ダウンロードする）。更新は `external/android-kiosk/tools/release-apk.sh` →
コミット → デプロイ（`external/android-kiosk/README.md`「配布」参照）。
