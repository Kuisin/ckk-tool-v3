...
- 出现不良品时，可以留下种类、根数和详情。
- 在检查工序中，可以记录测量值和合格与否。
- 当多份指示书共用同一道工序时，可以 **批量开始、暂停或完成**。
- 同时进行多道工序时，可以随时在画面一角查看当前进行中的工序。
...
如果同一台机器要连续处理同一道工序的多份指示书，不必逐个打开，可以批量开始、暂停或完成。

### 在列表中选择并批量操作

1. 当列表中有 2 道以上工序时，右上角会出现「**選択**」（选择）按钮。点击后，卡片上会出现复选框。
2. 勾选想要批量操作的工序。当前状态下无法执行该操作的工序，复选框会保持禁用（原因可通过状态标签判断）。
3. 画面下方的条形区域会显示已选数量，以及所选工序 **全部共通的操作**（开始、暂停、完成）按钮。
4. 点击按钮会打开确认画面，列出可以批量处理的工序，以及无法批量处理的工序（附带原因）。无法批量处理的工序可通过「**開く**」（打开）单独处理。
5. 点击「**実行**」（执行）即可批量处理。

> ⚠️ **可开始的工序与进行中的工序不能同时勾选。** 如果出现「開始できる工程と作業中の工程が混ざっています。どちらかに揃えてください。」（可开始的工序与进行中的工序混在一起。请只选择其中一类。）的提示，请重新只勾选其中一类。

> ⚠️ 批量完成时，**接收数会全部按良品记录。** 有不良的工序请不要放入批量对象，改为单独打开录入明细。同样，批量开始时的接收数会保持从上道工序带过来的预计值不变，如需修改也请单独打开该工序。

> 💡 **从暂停状态继续（恢复）无法批量进行。** 需要恢复的工序请逐个单独打开操作。

执行后画面会切换为结果显示。失败的工序会保持选中状态，方便直接修正后再次执行。

### 一起打开同一道工序

当列表中出现 **同一道工序** 2 件以上时，该组上方会自动出现一行「**{工序名称} — n 件**」，并附有「**まとめて開く**」（一起打开）按钮。点击后会打开一个只集合该工序的专用画面。

这个画面有列表批量操作所没有的优势：**可以为每份指示书分别输入不同的批次/单据编号，再一起开始。**（列表侧的批量开始会把需要批次/单据编号的工序排除在外，提示单独打开；而在这里，每一行都有自己的输入框。）

1. 在「**開始できる工序**」（可开始的工序）中，按需为每一行输入批次/单据编号。
2. 勾选想要一起开始的行，点击「**選択した n 件を開始**」（开始所选 n 件）。
3. 进行中和已暂停的工序会显示在「**作業中・一時停止中の工程**」（进行中 / 已暂停）栏中。勾选想要一起完成的行，点击「**選択した n 件を完了**」（完成所选 n 件）（这里的完成同样是 **全数按良品处理**；有不良的工序请通过「**開く**」（打开）单独录入）。
4. 当前无法操作的工序（等待上道工序、正被他人作业中等）只会在「**いま操作できない工程**」（当前无法操作）中列出，供查看。

点击「**工程一覧へ**」（返回工序列表）即可回到原来的列表。

## 随时查看进行中的工序

当你同时进行多道工序时，画面右下角会一直显示一个圆形按钮「**作業中 n件**」（进行中 n 件）。不必返回列表，也能随时确认自己当前手上有哪些工序。

- 点击它，进行中的工序会以卡片形式列出，可查看工序名称、指示书编号和经过时间。
- 点击卡片会直接跳转到该工序的记录画面。
- 没有进行中的工序时不会显示。

当同时进行 2 道以上工序时，会出现提示「**n 件同時のため、実働時間は 1/n で計上されます**」（同时 n 件，实际工时按 1/n 计入）。这与「[开始作业](#开始作业)」中提到的同时作业按比例计入工时的规则相同。
...
**Q. 选了好几道工序，却没有出现开始或完成按钮。**
A. 可能是可开始的工序与进行中的工序混在了一起。请重新只勾选其中一类。

**Q. 批量完成后，没有地方可以录入不良。**
A. 批量完成会把接收数全部按良品记录。请把有不良的工序从批量对象中去掉，单独打开录入明细。

**Q. 出现「ロット/伝票コードが必要です」（需要批次/单据编号），无法批量开始。**
A. 批次/单据编号每份指示书都不同，列表的批量开始无法处理。请单独打开该工序，或者如果同一道工序有多份，使用「**まとめて開く**」（一起打开）画面，在每一行输入编号后再开始。
