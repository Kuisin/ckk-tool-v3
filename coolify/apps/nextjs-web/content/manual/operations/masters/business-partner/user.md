...
- **過不足納品（かふそくのうひん）** … 受注した数量とぴったり合わない本数で[出荷](/manual/ja/operations/shipping/delivery-order/user)することです。どこまでのずれを受け取るかは、顧客ごとにこの画面の「過不足納品」欄で決めます。
...
3. 「**与信限度額**」があれば入れます。
4. 「**課税区分**」を選びます。空欄のままなら**製品ごとの税区分に従います**（ふつうはこれで構いません）。ここに設定すると、製品の区分より優先されます（非課税のお取引先など）。
...
### 4. 過不足納品の許容範囲を決める（「顧客」にチェックしたとき）

続けて「**過不足納品**」という区画が出てきます。ここでは、受注した数量とぴったり合わない本数で[出荷](/manual/ja/operations/shipping/delivery-order/user)してよいか、そのときは承認を挟むかを決めます。

> ⚠️ **この設定だけでは過不足納品はできるようになりません。** 実際に過不足のまま出荷できるかどうかは、[指示書](/manual/ja/operations/production/work-order/user)側で「過不足のまま出荷してよい」と許可されたロットに限られます。ここで決めるのは、そのロットについて「この顧客がどこまでのずれを受け取ってくれるか」と「承認が要るか」だけです。

1. 「**許容の基準**」で、ずれの幅を「**パーセント（%）**」で決めるか「**数量**」で決めるかを選びます。
2. 「**不足の許容**」に、受注数量を下回ってよい幅を入れます。空欄のままだと「不足はいっさい認めない」という扱いになります。
3. 「**超過の許容**」に、受注数量を上回ってよい幅を入れます。こちらも空欄は「超過はいっさい認めない」です。
4. 過不足があっても出荷書の確定前に必ず承認を通したいときは、「**許容範囲内でも承認を要求する**」にチェックを入れます（既定はチェックなしで、範囲内の過不足は承認なしで確定できます）。
5. 「**許容範囲外は承認を要求する**」は既定でチェックが入っています。外すと、幅を超えた過不足も承認なしで確定できてしまうため、よほどの理由がない限りそのままにしてください。

> 💡 「許容の基準」を「パーセント（%）」にして、受注 100 本に対して「不足の許容」を「2」に設定した場合、98 本まで（2 本不足）は範囲内、97 本以下は範囲外という扱いになります。

### 5. 最終需要家情報を入れる（「最終需要家」にチェックしたとき）

「**最終需要家情報**」という欄が出てきます。「**業種**」に、その会社の事業分野を入れます（例：自動車部品）。決まった選択肢はなく、社内で分かる言葉で構いません。空欄でも保存できます。

### 6. 仕入先・外注先の情報と振込先を入れる（「仕入先・外注先」にチェックしたとき）
...
一覧で行をクリックすると、その会社の画面が開きます。上のまとめ欄に **ロール** が表示され、その下は 5 つのタブに分かれています。

![取引先の詳細画面](../../../assets/screenshots/master-bp-detail-01.png)

- **概要** … 枠がいくつかに分かれて並びます。いちばん上が「**一般**」（備考など、ロールに関係なくその会社そのものの情報）、続いて **付いているロールの枠だけ** が 1 つずつ並びます。ロールの枠は「**顧客**」なら取引条件、「**最終需要家**」なら業種、「**仕入先・外注先**」なら取引条件と振込先が入ります。枠の見出しの左にある丸い印は、一覧のロールのバッジと同じ色です。
- **担当者** … 先方の窓口の方の一覧です。
- **支店一覧** … この会社の支店の一覧です。
...
取引先は、**どの会社にも共通の欄**と、**ロール（顧客・仕入先や外注先・最終需要家）を付けたときだけ出る欄**に分かれています。

### 共通の入力項目

| 項目 | 必須 | 何を入れるか |
|------|------|-------------|
| [BP コード](#field-bp-code) | 自動 | 取引先の番号 |
| [名称](#field-name) | 必須 | 会社名 |
| [フリガナ / 略称](#field-name-kana) | 任意 | 探すため・画面に短く出すための名前 |
| [国](#field-country) | 任意 | 相手の国 |
| [法人番号](#field-tax-number) | 任意 | 法人番号など |
| [AI 照合名](#field-match-names) | 任意 | 書類から読み取るときの別表記 |
| [郵便番号 / 住所](#field-address) | 任意 | 所在地 |
| [電話番号 / FAX / メールアドレス / Web サイト](#field-contact) | 任意 | 連絡先 |
| [有効](#field-active) | — | 選択肢に出すかどうか |
| [備考](#field-notes) | 任意 | 補足 |

### BP コード [#field-bp-code]

取引先の番号です。自動で付きます。

### 名称 [#field-name]

会社名です。**書類にそのまま出ます**ので、正式名称を入れてください。

### フリガナ / 略称 [#field-name-kana]
...
受け取った書類を自動で読み取るときに、**同じ会社だと判断させたい別表記**です。旧社名や、「株式会社」の付け方が違う書き方などを並べておくと、取り込みで会社を取り違えにくくなります。

### 郵便番号 / 住所 [#field-address]

所在地です。納品書や請求書に出ます。

### 電話番号 / FAX / メールアドレス / Web サイト [#field-contact]

会社としての連絡先です。担当者ごとの連絡先は、別に「担当者」として登録します。

### 有効 [#field-active]

外すと、見積書や発注書などで**新しく選べなくなります**。すでにその会社で作った書類はそのまま残ります。取引をやめた会社は、消さずにここを外してください。
...
| 項目 | 必須 | 何を入れるか |
|------|------|-------------|
| [請求先（別法人の場合）](#field-billing-bp) | 任意 | 請求書の宛先が別の会社のとき |
| [締日 / 支払サイト / 支払日](#field-payment-terms) | 任意 | 入金の約束 |
| [与信限度額](#field-credit-limit) | 任意 | 掛けで受けられる上限 |
| [課税区分](#field-tax-type) | 任意 | 消費税の扱い |
| [請求書送付方法](#field-invoice-method) | 任意 | 請求書をどう届けるか |
| [委託先](#field-consignment) | — | 委託販売の対象かどうか |
| [許容の基準](#field-tolerance-basis) | 任意 | 過不足の幅をパーセントで測るか数量で測るか |
| [不足の許容 / 超過の許容](#field-tolerance-range) | 任意 | 受注数量からどこまでずれてよいか |
| [許容範囲内でも承認を要求する / 許容範囲外は承認を要求する](#field-variance-approval) | — | 過不足のとき出荷書の確定前に承認を挟むか |

### 請求先（別法人の場合） [#field-billing-bp]

請求書の宛先が別の会社のときに指定します。空なら、その会社自身が宛先です。

### 締日 / 支払サイト / 支払日 [#field-payment-terms]

入金の約束です。**締日処理は、ここで決めた締日ごとに動きます。** 支払サイトと支払日から、請求書の支払期限が決まります。

### 与信限度額 [#field-credit-limit]

掛けで受けられる金額の目安です。

### 課税区分 [#field-tax-type]

消費税の扱いです。請求書・見積書の計算に使われます。

**空欄 = 製品に従う**。製品マスタの「税区分」で決まります。ここに値を入れると、その取引先の書類は製品の区分に関わらずこの区分で計算されます。

### 請求書送付方法 [#field-invoice-method]

請求書をメール・FAX・郵送・ポータルのどれで届けるかです。

### 委託先 [#field-consignment]

委託販売の対象かどうかです。委託販売をしている相手にだけ付けます。

### 許容の基準 [#field-tolerance-basis]

[過不足納品](/manual/ja/operations/shipping/delivery-order/user)の不足・超過の幅を「**パーセント（%）**」で測るか「**数量**」で測るかです。受注ごとに本数が大きく変わる顧客はパーセント、いつも近い本数で発注する顧客は数量のほうが合わせやすくなります。

### 不足の許容 / 超過の許容 [#field-tolerance-range]

受注数量からどこまでずれてよいかです。不足と超過で別々の幅を持てるので、「多く納品されるのは構わないが、足りないのは困る」という設定もできます。**どちらも空欄は「0 = 認めない」と同じ扱いです。**

### 許容範囲内でも承認を要求する / 許容範囲外は承認を要求する [#field-variance-approval]

過不足があったとき、[出荷書](/manual/ja/operations/shipping/delivery-order/user)を確定する前に承認を挟むかどうかです。**許容範囲の内側**と**外側**で別々に決められます。既定は「内側は承認不要・外側は承認必要」で、たいていの顧客はこのままで問題ありません。

---

### 仕入先・外注先ロールを付けたときの入力項目

| 項目 | 必須 | 何を入れるか |
|------|------|-------------|
| [外注種別](#field-vendor-type) | 必須 | 素材を買う相手か、工程を頼む相手か |
| [標準リードタイム（日数）](#field-lead-time) | 任意 | 頼んでから戻るまでの目安 |
| [締日 / 支払サイト / 支払日](#field-vendor-payment) | 任意 | 支払いの約束 |
| [銀行名 / 支店名 / 口座種別 / 口座番号](#field-bank) | 任意 | 振込先 |

### 外注種別 [#field-vendor-type]

**仕入先**（素材や資材を買う相手）か、**外注先**（工程の一部を頼む相手）かです。ここで選んだ側の画面にだけ、その会社が出ます。

### 標準リードタイム（日数） [#field-lead-time]

頼んでから戻ってくるまでの目安の日数です。外注の入荷予定日を決めるときの目安になります。

### 締日 / 支払サイト / 支払日 [#field-vendor-payment]
...
| 項目 | 必須 | 何を入れるか |
|------|------|-------------|
| [業種](#field-industry) | 任意 | どんな業種の会社か |

### 業種 [#field-industry]

どんな業種の会社かです。最終需要家は**大口のところだけ登録すれば十分**で、すべての需要家を登録する必要はありません。

---

### 支店・担当者の入力項目

支店は会社の下に登録します（**2 階層まで**）。入れる欄は共通の入力項目と同じです。

担当者には、名前のほか部署・役職・メール・電話を入れられます。**主担当**に印を付けておくと、書類の宛先として最初に出ます。
...
**Q.「過不足納品」の設定を緩めれば、受注数量と違う数量でも出荷できますか。**
A. これだけでは出荷できません。過不足のまま出荷できるかどうかは、[指示書](/manual/ja/operations/production/work-order/user)側で「過不足のまま出荷してよい」と許可されたロットに限られます。ここで決めているのは、その許可されたロットについて「どこまでのずれを受け取るか」と「承認が要るか」だけです。

**Q. 不足の許容・超過の許容を空欄のままにするとどうなりますか。**
A. その側の過不足はいっさい認められません（0 本まで、と同じ扱いです）。
