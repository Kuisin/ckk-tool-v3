...
- **過不足納品（交货数量差异）** … [出货](/manual/zh/operations/shipping/delivery-order/user)的数量与订货数量不完全一致。能接受多大的差异，按客户在本画面的「過不足納品」区域分别设定。
...
4. 选择「**課税区分**」（课税区分）。留空则**按各产品的税种**（通常这样即可）。在此设置后将优先于产品——例如免税的业务伙伴。
...
### 4. 设定交货数量差异的允许范围（勾选「顧客」时）

接着会出现「**過不足納品**」（交货数量差异）区域。这里决定能否以与订货数量不完全一致的数量[出货](/manual/zh/operations/shipping/delivery-order/user)，以及此时是否需要经过审批。

> ⚠️ **仅凭这项设置并不能让差异出货生效。** 能否以差异数量实际出货，仅限于[工单](/manual/zh/operations/production/work-order/user)一侧已被允许「可以在数量有差异的情况下出货」的批次。这里决定的只是针对那个已获许可的批次，「这位客户能接受多大的差异」以及「是否需要审批」。

1. 在「**許容の基準**」（允许基准）中，选择差异幅度按「**パーセント（%）**」（百分比）还是按「**数量**」（数量）计算。
2. 在「**不足の許容**」（不足允许范围）中填入允许低于订货数量的幅度。留空表示「完全不允许不足」。
3. 在「**超過の許容**」（超出允许范围）中填入允许高于订货数量的幅度。同样，留空表示「完全不允许超出」。
4. 如果希望即使差异在此范围内，出货单确定前也一定要经过审批，请勾选「**許容範囲内でも承認を要求する**」（在允许范围内也需要审批）（默认不勾选，范围内的差异可无需审批直接确定）。
5. 「**許容範囲外は承認を要求する**」（超出允许范围时需要审批）默认为勾选状态。取消勾选后，超出范围的差异也会无需审批直接通过，因此除非有充分理由，请保持勾选。

> 💡 将「許容の基準」（允许基准）设为「パーセント（%）」（百分比），并对 100 件的订货将「不足の許容」（不足允许范围）设为「2」时，交货到 98 件（不足 2 件）为止都算范围内，97 件以下则算超出范围。

### 5. 输入最终需求方信息（勾选「最終需要家」时）

会出现「**最終需要家情報**」（最终需求方信息）区域。请在「**業種**」（行业）中填入该公司的业务领域（例如：汽车零部件）。没有固定选项，用公司内部能理解的说法即可，留空也能保存。

### 6. 输入供应商・外协厂信息和汇款账户（勾选「仕入先・外注先」时）
...
| [允许基准](#field-tolerance-basis) | 选填 | 差异幅度按百分比还是按数量计算 |
| [不足允许范围 / 超出允许范围](#field-tolerance-range) | 选填 | 交货数量可以偏离订货数量的幅度 |
| [在允许范围内也需要审批 / 超出允许范围时需要审批](#field-variance-approval) | — | 出货单确定前是否需要审批 |
...
消费税的处理方式，用于请款单和报价单的计算。

**留空 = 按产品**。由产品主数据的「税区分」决定。在此填写后，该业务伙伴的单据一律按此税种计算，与产品无关。
...
### 允许基准 [#field-tolerance-basis]

[交货数量差异](/manual/zh/operations/shipping/delivery-order/user)的不足・超出幅度，按「**パーセント（%）**」（百分比）还是按「**数量**」（数量）计算。订货量波动较大的客户适合用百分比，每次订货量相近的客户用数量更容易对齐。

### 不足允许范围 / 超出允许范围 [#field-tolerance-range]

交货数量可以偏离订货数量的幅度。不足与超出可以分别设定不同的幅度，因此可以设置为「多交一些没关系，但不能少交」或相反的情况。**两者留空都视为「0 = 不允许」。**

### 在允许范围内也需要审批 / 超出允许范围时需要审批 [#field-variance-approval]

出现数量差异时，[出货单](/manual/zh/operations/shipping/delivery-order/user)确定前是否需要经过审批，可分别针对**范围内**和**范围外**设定。默认「范围内无需审批・范围外需要审批」对大多数客户已经够用。
...
**Q. 放宽「過不足納品」（交货数量差异）的设置后，是否就能以与订货数量不同的数量出货？**
A. 仅凭这项设置无法出货。能否以差异数量实际出货，仅限于[工单](/manual/zh/operations/production/work-order/user)一侧已被允许「可以在数量有差异的情况下出货」的批次。这里决定的只是针对那个已获许可的批次，「能接受多大的差异」以及「是否需要审批」。

**Q. 把「不足の許容」（不足允许范围）和「超過の許容」（超出允许范围）留空会怎样？**
A. 对应一侧完全不允许出现差异（等同于设为 0）。
