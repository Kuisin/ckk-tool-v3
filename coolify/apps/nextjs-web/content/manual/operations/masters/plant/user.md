...
製品在庫・素材在庫は[在庫管理](/manual/ja/operations/inventory/inventory-management/user)で拠点ごとに分けて管理され、素材の入荷先などもここで登録した拠点から選びます。
...
- 拠点の中の **倉庫や棚** は、このアプリでは登録しません。[保管場所](/manual/ja/operations/masters/storage-location/user)で登録します。このアプリで扱うのは図面だけです。
...
図面の登録・差し替えは、このアプリだけで行います。[保管場所](/manual/ja/operations/masters/storage-location/user)の画面では、この図面の上に印を置くことしかできません。
...
| 項目 | 必須 | 何を入れるか |
|------|------|-------------|
| [拠点コード](#field-code) | 必須 | 拠点の管理番号 |
| [名称 / よみがな](#field-name) | 必須 | 拠点の名前 |
| [国 / 地域](#field-region) | 任意 | 権限の範囲に使われる区分 |
| [郵便番号 / 住所](#field-address) | 任意 | 所在地 |
| [電話番号 / メールアドレス / 担当者](#field-contact) | 任意 | 連絡先 |
| [有効](#field-active) | — | 選択肢に出すかどうか |
| [備考](#field-notes) | 任意 | 補足 |

### 拠点コード [#field-code]

拠点の管理番号です。在庫や出荷の画面でこのコードが使われます。

### 名称 / よみがな [#field-name]

拠点の名前です。在庫・入荷・出荷の「どこの」を表します。

### 国 / 地域 [#field-region]

国と地域の区分です。**利用者の権限を「この地域の拠点だけ」と決めるときの単位**になります。
...
**Q. 倉庫や棚はどこで登録しますか。**
A. このアプリではなく、[保管場所](/manual/ja/operations/masters/storage-location/user)で登録します。
