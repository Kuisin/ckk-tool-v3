...
- 一覧は **コード / 名称 / カテゴリ / 工程リスト種別 / 実施場所 / 同期可 / 検査 / 承認 / 数量管理 / 表示順 / 状態** の順に並んでいます。「**工程リスト種別**」は「準備」か「製造」のバッジで、カテゴリから自動で決まります。
...
9. 「**作業計画の必須項目**」で、[指示書](/manual/ja/operations/production/work-order/user)の承認依頼を出す前にこの工程の作業計画へ何が要るかを決めます（次の「作業計画の必須項目」をご覧ください）。
10. 「**表示順**」に数字を入れます。小さい数字ほど一覧の上に出ます。
11. 「**保存**」を押します。
