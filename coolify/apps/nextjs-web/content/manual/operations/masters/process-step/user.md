...
- **工程リスト種別** … その工程が、どちらの工程リストに入るかです。**カテゴリが「材料準備」の工程だけ準備工程リスト（全製品で共通の 1 本）に入り**、それ以外のカテゴリの工程はすべて製造工程リスト（製品×受注元ごと）に入ります。カテゴリを選ぶと自動で決まり、ここだけを個別に選ぶことはできません。
- **実施場所** … その工程を社内だけで行うか、外の会社にもお願いできるかの区分です。
- **同期可（どうきか）** … ほかの工程と同時に進めて、まとめて記録してよい工程のことです。
- **数量管理** … 工程を行ったときに、現場の人に何の数を入力してもらうかの設定です。
- **作業計画の必須項目** … [指示書](/manual/ja/operations/production/work-order/user)の承認依頼を出す前に、この工程の作業計画へ何を入れておく必要があるかです。計画日は常に必須で、担当者・作業場所・開始/終了時刻・数量は工程ごとに要る/要らないを決められます。
...
- 一覧の右上、「**新規作成**」の隣にある「**準備工程リスト**」ボタンから、共通の準備工程リストを管理する画面を開けます（詳しくは「[準備工程リストを管理する](#準備工程リストを管理する)」）。
...
- **作業計画に数量が必要** … 計画を数量で分割する工程（担当者ごとに何本、など）で入れます。
...
上のほうに、工程コード・名称・カテゴリ・**工程リスト種別**（準備 / 製造のバッジ）・実施場所・数量管理・ロット入力・**作業計画の必須項目**（担当者・日付・作業場所・時刻・数量のうち、実際に求められているもの）・既定作業時間・表示順などがまとめて表示されます。下には 3 つのタブがあります。
...
材料準備・加工・コーティング・検査・検査承認・出荷の区分です。**「材料準備」を選んだ工程だけ、共通の準備工程リストに入ります**（[準備工程リストを管理する](#準備工程リストを管理する)を参照）。それ以外のカテゴリの工程は、製品×受注元ごとの製造工程リストに入ります。
...
- **作業計画に数量が必要** … 既定は任意。担当者ごとに何本、のように計画を数量で分割する工程で立てます。
