...
「関連」（关联）标签页会列出该产品的设计图纸。

**版本按「产品 × 受订方」分别计数。** 即使是同一个产品，不同客户的图纸也会各自演进，所以 A 公司的 v3 和 B 公司的 v1 会并列在同一个产品下。没有受订方的系列是 **汎用（通用）**，供没有专属图纸的客户的指示书使用。每个系列都有自己的标题，最上方是最新版的缩略图（点击可放大，3D 模型可以直接旋转查看）。

每个版本都带有来源标签。

- **依頼（委托）** … 由[设计委托](/manual/zh/operations/sales/design-request/user)完成后生成的版本。
- **手動（手动）** … 直接从本画面添加的版本。

### 不经过设计委托直接添加图纸

图纸已经先做好了，或者要导入从别处收到的图纸时，可以用「**設計図を追加**」（添加设计图）直接登记。

1. 在「関連」标签页点击「**設計図を追加**」。
2. 选择 **受注元（受订方）**。留空则为通用。
3. 把文件放入 **図面データ（图纸数据，必填，1 份）**、**プレビュー用（预览用，选填，1 份）**、**参考資料（参考资料，选填，可多份）**。参考资料可以逐份填写说明。
4. 点击「**登録**」（登记）。

版本号接着该系列继续。即使同一产品下其他客户已有版本，某个客户的第一份图纸仍然是 v1。
...
**指示书指向的版本不能变动。** 那是「用这张图做了（要做）东西」的记录，如果之后内容被改动，就无法追溯当时到底看的是什么。在指示书侧解除固定后，就又可以修改了。

设计委托生成的版本不能删除是另一个原因——它就是已完成委托的成果物本身（备注仍可编辑）。

**图纸文件本身不能替换。** 修改图纸就意味着做一个新版本，请通过「設計図を追加」或完成设计委托来登记新版本。
