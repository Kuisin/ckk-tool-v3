...
Stock is kept separately down to "which shelf in which storage location it is on". In the stock transfer of [inventory management (在庫管理)](/manual/en/operations/inventory/product-inventory/user), you can move things between the places registered here.
...
| [作業場所 (Work location)](/manual/en/operations/masters/work-location/user) | A place or machine inside the site where work is done | NC lathe no. 1, polishing area |
...
- **Floor map** … the drawing of a floor of the site. The drawing itself is registered in [site (拠点)](/manual/en/operations/masters/plant/user).
- **Pin** … the mark placed on the drawing that shows where a storage location is.

## Before you start

- You need the **master permission** to use this app.
- A storage location always belongs to one of the sites. Please register the [site (拠点)](/manual/en/operations/masters/plant/user) first.
- If you want to place a mark on a drawing, first register the drawing on the 「フロアマップ」 (floor maps) tab of the [site (拠点)](/manual/en/operations/masters/plant/user). You cannot add or swap drawings in this app.
...
A. There is still stock in that place. Either move the contents to another place with a stock transfer in [inventory management (在庫管理)](/manual/en/operations/inventory/product-inventory/user) to empty it, or turn off 「**有効**」 (active) on the edit screen to make it inactive. It is the same when you delete a shelf.
...
A. Registering and swapping drawings is done in the [site (拠点)](/manual/en/operations/masters/plant/user) app. In this app you can only place marks on a drawing that is already registered.
