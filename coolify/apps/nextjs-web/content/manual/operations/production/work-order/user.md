...
5. 「**種別**」で「在庫分」か「製造分」を選びます（注文種別が「再研磨」の明細では「**再研磨**」に固定されます — 「[再研磨の指示書](#再研磨の指示書)」を参照）。在庫分は明細を 1 つだけ選べます。**在庫分の工程は「製品出し（在庫）」＋（必要なら）「出荷前検査」の固定構成**で、工程リストは使いません — 以降の 11〜13（準備工程リスト・製造工程リスト・実施場所やロット入力の設定）は製造分だけの手順です。
...
工程の並びは「工程リスト」として登録されています（**製造分のみ**。在庫分は固定構成のため工程リストを使いません）。工程リストは **準備工程リスト**（共通）と **製造工程リスト**（製品 × 受注元）の 2 本があり、指示書はこの 2 本を合わせて 1 つの工程の並びをつくります。
...
- **使用素材・保管場所はありません。** 工程は「[再研磨工程リスト](#field-regrind-route)」から流し込みます（材料準備・加工の工程は選べません）。
...
〇〇出し・受渡しと材料準備の並びです。製品にも受注元にも紐づかない、全製品で共通の 1 本を選びます。**バージョンは選べません** — 選んだリストの常に最新版が使われます。有効な準備工程リストが 1 本もあれば必須で、1 本しかなければ自動で選ばれます。在庫分の指示書では使いません（固定構成のため）。並びを直すときは、指示書の側ではなく[工程マスタの準備工程リスト](/manual/ja/operations/masters/process-step/user#準備工程リストを管理する)で新しい版をつくります。
...
加工以降の工程の並びです。製品 × 受注元ごとに登録された製造工程リストから選びます。**バージョンを選ぶと、その時点の並びがそのまま複製されます** — 後で工程リストを直しても、作成済みの指示書は変わりません。在庫分の指示書では使いません（固定構成のため）。
