...
- **入口は注文請書です。** 注文種別が「**再研磨**」の明細から作ります（[未処理指示書](/manual/ja/operations/production/pending-work-order/user)や注文明細の画面の「指示書を作る」から開くと、種別は「再研磨」に固定されています）。他社製の工具は、先に[製品マスタ](/manual/ja/operations/masters/product/user)で「**他社製品**」として登録しておきます。
...
- **完了しても自社の在庫は増えません。** 詳細画面の「再研磨数量」に 受入 / 返却 / 完成 の 3 つの本数が出ます。研ぎ直した本数は[出荷書](/manual/ja/operations/shipping/delivery-order/user)（発送）でお客様へ返し、単価は価格表の「再研磨」の価格が使われます。
