...
5. 「**種別**」で「在庫分」か「製造分」を選びます。在庫分は明細を 1 つだけ選べます。**在庫分の工程は「製品出し（在庫）」＋（必要なら）「出荷前検査」の固定構成**で、工程リストは使いません — 以降の 11〜13（準備工程リスト・製造工程リスト・実施場所やロット入力の設定）は製造分だけの手順です。
...
- **外注日程** … 外注の工程では「依頼日」「入荷予定日」「入荷日」「外注費」を記録できます。**依頼日を入れると、その本数は「外注先が預かっている分」として記録されます**（[在庫一覧](/manual/ja/operations/inventory/stock-overview/user)の「預け先」で見られます）。入荷日を入れると戻ります。どちらも[入出庫伝票](/manual/ja/operations/inventory/movements/user)が 1 枚できて、工程の画面から番号をたどれます。預けている分は自社の在庫には数えないので、出荷にも引当にも使われません。
...
在庫は[在庫管理](/manual/ja/operations/inventory/product-inventory/user)（`ST01`）で確認できます。作っている途中の本数は、同じアプリの「**仕掛品**」タブで見られます。

完了のときに在庫が動いたことは[入出庫伝票](/manual/ja/operations/inventory/movements/user)に 1 枚の伝票として残ります（事由「指示書完了」）。**半製品から作った指示書では、使った半製品がこのときに在庫から減ります。**
