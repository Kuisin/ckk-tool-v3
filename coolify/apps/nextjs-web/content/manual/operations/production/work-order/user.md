...
5. 「**種別**」で「在庫分」か「製造分」を選びます（注文種別が「再研磨」の明細では「**再研磨**」に固定されます — 「[再研磨の指示書](#再研磨の指示書)」を参照）。在庫分は明細を 1 つだけ選べます。**在庫分の指示書は工程リストを使わず、工程を直接選びます**。必ず「製品出し（在庫）」で始まり、ほかにどの工程を選べるかは[工程マスタ](/manual/ja/operations/masters/process-step/user#使える指示書種別の決めかた)の「使える指示書種別」で決まります（はじめは「出荷前検査」だけです）。以降の 11〜13（準備工程リスト・製造工程リスト・実施場所やロット入力の設定）は製造分だけの手順です。
...
- **入口は注文請書です。** 注文種別が「**再研磨**」の明細から作ります（[未処理指示書](/manual/ja/operations/production/pending-work-order/user)や注文明細の画面の「指示書を作る」から開くと、種別は「再研磨」に固定されています）。再研磨の明細は品目を 2 つ指していて、**売っているのは研ぎ直しという役務**、**指示書の「製品」になるのは研ぎ直す工具のほう**です。役務は[再研磨品目](/manual/ja/operations/masters/regrind-item/user)に、工具は[製品マスタ](/manual/ja/operations/masters/product/user)に登録しておきます（他社が作った工具は「**他社製品**」として登録します）。
