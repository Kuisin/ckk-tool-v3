...
14. 「**作業計画（担当者）**」で、工程ごとの担当者・計画日・（作業場所を求める工程では）作業場所を入力します。**担当者は工程マスタで「担当者必須」にされていない限り任意です** — 空のままにしておいて、現場で後から決めることもできます。時刻・数量まで必須の工程では「この工程の計画には時刻・数量も必要です」と出るので、作成後に工程ごとの計画パネルで追加してください。
15. 「**保存**」を押します。
...
- **どの項目が必須かは工程マスタが工程ごとに決めます。** 計画日は常にどの工程でも必須です。担当者・作業場所・開始/終了時刻・数量は、[工程マスタ](/manual/ja/operations/masters/process-step/user#field-plan-required-fields)でその工程に必須と設定されているときだけ赤いアスタリスクが付きます。**担当者は既定では任意** — 承認を出すためだけに仮の担当者を入れる必要はなく、承認後に現場で決められます。作業実績のほうは（誰がやったかの記録なので）担当者・日付が常に必須です。
...
工程ごとの担当者・計画日・（工程マスタが求める工程だけ）作業場所です。**必須になるかどうかは工程マスタが工程ごとに決めます**（計画日だけは常に必須）。担当者は既定では任意で、空のままでもかまいません — 入れると作業計画が作られ、担当者の現場タブレットの「工程実行」の一覧にその作業が出ます。時間帯・数量まで決める場合は、作成後に各工程の計画の表で追加してください。承認依頼を出すには、社内のすべての工程に、必須項目が揃った計画が最低 1 行必要です（上の「承認をもらう」を参照）。
