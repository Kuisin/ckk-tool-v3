...
- **「主図面」（主图纸）** … 该版本的代表文件。产品主数据的「最新图纸」指的就是它。
- **「参考資料」（参考资料）** … 与主图纸同属一个版本的其他文件（零件图、尺寸表、3D 模型等）。
...
1. 点击画面右上角的「**…**」。
2. 选择「**完了**」（完成）。
3. 在确认画面里勾选 **要放进这个版本的文件**，并选出 **主図面（主图纸）**。不够时可以用「**ファイルを追加**」（添加文件）当场上传。
4. 点击「**完了**」（完成）。

**一次完成 = 一个版本。** 选中的文件都会得到同一个版本号（v1、v2 …），其中 **主図面** 成为产品主数据的最新图纸，其余作为 **参考資料（参考资料）** 放进同一版本。装配图、零件图、3D 模型一起提交时版本号不会各自分开，同时记录完成日期，并通知提出委托的人（报价时提出的，还会通知该报价的营业负责人）。指定了产品的委托，该产品的最新图纸也会切换到这个版本。
