...
- 负责人点击「着手」（开始）变成 **「進行中」（进行中）**，附上图纸并点击「完了」（完成）就变成 **「完了」（完成）**。
...
2. 点击右上角的「**アップロード**」（上传）。
3. 选择文件（**没有格式限制** —— PDF、图片、3D 模型、Excel 等。每个最大 20MB）。
...
只有在 **审批通过之后到完成之前**（未着手・進行中）才能添加附件。

### 在画面上查看图纸

已登记的版本可以从文件名旁边的「**表示**」（显示）在应用内打开。
PDF 和图片直接显示，**STL 等 3D 模型可以转动查看**（拖动旋转，滚轮缩放）。

STEP、IGES、DXF、DWG 在这里打不开，请下载后用您自己的软件查看。

## 完成委托

1. 点击画面右上角的「**…**」。
2. 选择「**完了**」（完成）。
3. 确认画面上有 **図面データ（图纸数据，必填，1 份）**、**プレビュー用（预览用，选填，1 份）**、**参考資料（参考资料，选填，可放多份）** 三种框。每个框都可以用「**ファイルを選択**」（选择文件）放入文件。
4. 参考资料可以逐份填写**说明**（如「部品図」「寸法表」），之后查看版本列表时就能知道每份文件是什么图。
5. 点击「**完了**」（完成）。上传在这时才开始。

**上传的文件一定会归入其中一个角色。** 不能不指定角色就先放着。

**一次完成 = 一个版本。** 选中的文件都会得到同一个版本号（v1、v2 …）。

**版本按「产品 × 受订方」分别计数。** 即使是同一个产品，不同客户的图纸也会各自演进，所以 A 公司的 v3 和 B 公司的 v1 可以同时存在于一个产品上。完成的版本落在哪个系列，由委托的 **受注元（受订方）** 决定；留空则为 **汎用（通用）**，供没有专属图纸的客户的指示书使用。

**预览用和图纸数据要分开选**，因为用途不同：同样的形状，STL 是「用来看的」，CAD 是「用来做的」，彼此不能替代。产品主数据的最新图纸指向 **図面データ**，画面上的缩略图则优先显示 **プレビュー用**。装配图、零件图、3D 模型一起提交时版本号不会各自分开，同时记录完成日期，并通知提出委托的人（报价时提出的，还会通知该报价的营业负责人）。指定了产品的委托，该产品的最新图纸也会切换到这个版本。

完成之后如果还需要修改，请从「**…**」点击「**差し戻し**」（退回），再点击「**差し戻す**」（退回）。状态会回到进行中，可以再次添加文件（完成日期会被清除）。这个退回是 **重做作业**，不需要重新走一遍审批。
