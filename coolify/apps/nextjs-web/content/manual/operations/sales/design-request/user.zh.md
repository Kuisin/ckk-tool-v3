...
1. 点击画面右上角的「**…**」。
2. 选择「**完了**」（完成）。
3. 确认画面上有 **図面データ（图纸数据，必填）**、**プレビュー用（预览用，可选）**、**参考資料（参考资料，可选，可放多份）** 三种框。每个框都可以用「**ファイルを選択**」（选择文件）直接放入文件。作业中已经上传到文件标签页的文件，可以从框下方的「**または、添付済みから選ぶ**」（或从已添加的附件中选择）选取。
4. 点击「**完了**」（完成）。上传在这时才开始。

同一个文件不能放进两个框（一个文件 = 一个角色）。
