...
- **基準単価** … その注文種別の基本の値段です。ふつうは[価格試算](/manual/ja/operations/sales/trial-estimate/user)の結果から入ります。
...
## 製品と再研磨は、同じこの画面で値段を付けます

売り物は 2 種類あり、**どちらもこの価格表で顧客ごとの値段を持ちます**。別のアプリには分かれていません。

| 売り物 | 品目を登録する場所 | 使う注文種別 |
|---|---|---|
| **製品** | [製品](/manual/ja/operations/masters/product/user)（MS04） | 本番 / テスト / サンプル / その他 |
| **再研磨**（研ぎ直しという役務） | [再研磨品目](/manual/ja/operations/masters/regrind-item/user)（MS0H） | 再研磨 |

一覧では、再研磨の行に **「再研磨」のバッジ** が付きます。工具の名前と「研ぎ直しという役務」の名前は似ているので、行がどちらかを見分けられるようにしています。「製品」の絞り込みにも再研磨品目が出ます。

> 💡 **組み合わせは決まっています。** 再研磨品目には「再研磨」の値段しか作れず、製品には「再研磨」の値段を作れません。売っているものが違うためで、間違えると保存のときに知らせます。

> ⚠️ **お客様から預かる工具そのもの（他社製品）には値段を付けません。** 預かって研ぎ直して返すだけで、売ってはいないからです。この画面の品目の選択肢にも出てきません。

### 標準価格はここでは直せません

再研磨品目は、顧客を問わない **標準価格**（定価）を持っています。この画面では **読むだけ** で、直すのは[再研磨品目](/manual/ja/operations/masters/regrind-item/user)（MS0H）です。値段の持ち主を 2 か所に分けないためで、詳細画面の「品目マスタで直す」から飛べます。

値段の決まり方は 2 段です。

1. 品目の **標準価格**
2. その顧客の **価格表** — あればこちらが勝ちます

**落ちるのは「当たる価格表が無い」ときだけ**です（作っていない / 無効 / 有効期間の外）。⚠️ 生きている価格表にその数量の段階が無いときは標準価格に落ちません — 設定の穴が見えなくなるので、単価が引けないままにします。段階を直してください。
...
- 値段のもとにする[価格試算](/manual/ja/operations/sales/trial-estimate/user)があると便利です。価格試算に製品を指定して「確定」しておくと、この画面で選べるようになり、金額が自動で入ります。
- 価格試算がなくても、値段を手で入力して登録できます。
...
- **価格試算元** … どの価格試算から値段を取り込んだかです。「手動」は手で入力したものです。
...
5. 「**価格ソース（価格試算）**」の欄をクリックし、値段のもとにする価格試算を選びます。選ぶと「**基準単価**」に金額が自動で入ります。
6. 「**有効開始日**」を選びます。
7. テストとサンプルの場合は「**有効終了日**」も必ず選びます。
8. 数量の表で「**最小数量**」「**最大数量**」「**倍率**」を入れます。
9. 区切りを増やしたいときは「**範囲を追加**」を押します。新しい注文種別を足すと、数量の区切りははじめから[数量スケール](/manual/ja/operations/sales/trial-estimate/settings)（管理者が決めた「何本から何倍か」の表）が入ります。「**プリセットに戻す**」を押すと、その注文種別の区切りをこの表の状態に戻せます（手で足した範囲やカスタム単価は消えます）。
...
> ⚠️ 「この製品にリンクされた確定済みの価格試算はありません」と出たときは、その製品の価格試算がまだ確定していません。価格試算を使いたい場合は先に[価格試算](/manual/ja/operations/sales/trial-estimate/user)で製品を指定して確定してください。価格試算を使わない場合は「**カスタム単価を使用**」にチェックを入れ、基準単価を手で入力します。

## 登録した内容を見る

価格表の画面には、5 つのタブがあります。

![価格表の詳細画面](../../../assets/screenshots/price-list-detail-01.png)

- **価格設定** … 注文種別ごとの基準単価・有効期間・数量ごとの単価の一覧です。
- **値引き設定** … 値引きルールの一覧です。
- **関連** … どの価格試算から値段が入ったか、この価格表から作った見積書の一覧です。
- **コメント** … この価格表についてのやり取りを投稿できるスレッドです。
- **履歴** … いつ誰が変更したかの記録です。
...
- 保存済みの注文種別と、その値段のもとにした価格試算も変えられません。
- ある数量の区切りだけ特別な値段にしたいときは、その行の「**カスタム**」ボタンを押し、金額を入力します。確認の画面が出るので「**カスタム設定する**」を押します。自動計算に戻すときは「**自動に戻す**」を押します。
...
- **別の顧客・製品へコピー** … 同じ値段の設定を、別のお客様や製品にコピーします（価格試算とのつながりは外れ、手で入力した扱いになります）。
...
| [価格ソース（価格試算）](#field-estimate) | 任意 | 基準単価のもとにする価格試算 |
...
使わなくなった価格表に「無効」の印を付けて整理しておくための項目です。無効にしても、いまのところ見積書の単価の計算には影響しません。

### 注文種別 [#field-order-type]

本番・テスト・サンプル・その他の区分です。**同じ顧客・製品でも、種別ごとに違う価格を持てます。** サンプルは単価を 0 に設定して使う運用です（自動で 0 になるわけではありません）。

### 価格ソース（価格試算）[#field-estimate]

基準単価のもとにする価格試算です。**確定した価格試算だけ選べます。** 選ぶとその価格試算の見積単価が基準単価に入り、価格試算は「価格表登録済」になって編集できなくなります（過去の価格が後から変わらないようにするためです）。手で入れたい場合は選ばなくても構いません。
...
この価格をいつから使い始めたかを記録しておく日付です。見積書の単価はこの日付では切り替わりません。期間で自動で効いたり止まったりするのは、値引きルール（値引きルール自身の期間）のほうです。
...
**Q. 基準単価の欄に数字を入れられません。**
A. 価格試算から値段を取り込んでいる場合、基準単価は価格試算の金額のまま使う決まりです。別の金額にしたいときは「**カスタム単価を使用**」にチェックを入れてください（確認の画面が出ます）。
