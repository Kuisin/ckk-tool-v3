...
---
这是管理员用来决定 [価格試算](/manual/zh/operations/sales/trial-estimate/user)（试算）应用 **如何计算单价** 的应用。操作码是 `SY02`。
...
点击主页「システム」（系统）中的 **価格試算計算**。或者在界面上方的搜索框中输入 `SY02`。
...
打开应用后会排列出 6 张卡片。点击卡片即可打开各自的设置界面。

![価格試算計算的首页界面](../../../assets/screenshots/trial-pricing-hub-01.png)

- 「**計算基準**」（计算基准）… 组成单价的部件一览。
- 「**工具種管理**」（工具种类管理）… 工具种类的添加、删除，以及各种类的设置。
- 「**材料参照価格ポリシー**」（材料参考价格策略）… 材料价格从哪里取得的设置。
- 「**数量スケール**」（数量阶梯）… 价格表数量区间与系数的初始表。
...
1. 在一览中点击想修改的基准。
2. 右侧会显示其内容。
3. 可在「**基準名**」（基准名）中修改显示名称。在「**説明**」（说明）中可写明该费用统计的内容；在价格试算画面点击各行旁的 **ⓘ** 即可查看（留空则不显示按钮）。
...
- 每一行会显示「**計算基準 N 件 · 価格試算 M 件**」（计算基准 N 件 · 试算 M 件）。这表示该种类使用了多少个基准，以及该种类的试算有多少件。
...
## 数量スケール（数量阶梯）— 价格表的初始数量区间

在价格表中新建订单类别时，数量区间和系数的初始值来自这张表。它取代了以前内置在价格试算里的“批量折扣率”，现在可在此自由设定。

- 每行只有「**開始数量**」（起始数量）和「**倍率**」（系数）。范围（到多少支）由下一行的起始数量决定，因此不会出现空隙或重叠。第一行从 1 开始。
- 「**単価**」（单价）列是示例：页面上方输入的「**試算用の基準単価**」（试算用基准单价）乘以系数（不会保存）。
- 点击「**範囲を追加**」（添加范围）可加行，点击「**初期値に戻す**」（恢复默认）可回到标准表。保存前不会生效。
- **已创建的价格表不会改变。** 只影响今后新建的订单类别的初始值。
...
**Q. 添加的工具种类删不掉。**
A. 说明还有使用该种类的试算。一览中的「価格試算 M 件」（试算 M 件）不为 `0` 时无法删除。不再使用的种类，即使不删除、就这样留着也没有问题。
