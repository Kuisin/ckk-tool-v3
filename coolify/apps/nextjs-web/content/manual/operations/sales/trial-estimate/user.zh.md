...
1. 打开作为基础的试算。
2. 从右上角的「**…**」中点击「**複製して作り直す**」（复制后重新试算）。
3. 会打开内容相同的新输入画面，只改想改的地方。
4. 点击「**保存**」（保存）。

试算名称会自动加上「（作り直し）」（重新试算），编号会编上新的号码。在列表某一行的「**…**」中也能做同样的操作。
...
**Q. 保存后的试算不能编辑。**
A. 试算的机制是把保存当时的价格作为记录保留下来，所以之后不能修改。请用「**複製して作り直す**」（复制后重新试算）重新做一份。
...
A. 已经用于价格表的试算（状态为「価格表登録済」）不能修改内容。想用别的产品计算时，请用「複製して作り直す」（复制后重新试算）重新做一份。
