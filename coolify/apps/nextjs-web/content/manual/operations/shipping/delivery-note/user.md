...
3. 「**納品先**」と明細（製品・数量・単価）が **自動で入ります**。あわせて、注文請書に登録されている配送方法がひとつに決まるときは「**納品方法**」（と価格記載の初期値）も、届け先の会社がひとつに決まるときは「**最終需要家**」も自動で入ります（決まらないときは空のままです）。
4. 「**納品方法**」を確認します。必要なら「**通常納品**」か「**ユーザー直送**」に切り替えます。
...
- 出荷書を選んだときに納品方法が自動で入った場合も、同じ規則で価格記載の初期値が決まります。
...
もとになる出荷書です。選ぶと、その出荷の製品・数量が引き継がれます。あわせて、納品方法・最終需要家・営業担当も、出荷書側からひとつに決められるときは自動で入ります（明細ごとに違っていて決まらないときは入りません）。
...
出荷書を選ぶと、その注文請書に登録されている配送方法がひとつに決まる場合は、ここに自動で入ります。
...
直送のときの実際の届け先です。大口のお客様のみ登録されています。出荷書を選んだとき、届け先の会社がひとつに決まる場合は自動で入ります（注文明細に指定があればそれを、なければ注文請書に登録された会社を使います）。
