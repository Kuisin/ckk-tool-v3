...
3. 「**納品先**」（送货对象）和明细（产品、数量、单价）会 **自动填好**。同时，订单回执上登记的配送方式能定为一种时，「**納品方法**」（送货方式）（以及记载价格的初始值）也会自动填入；收货公司能定为一家时，「**最終需要家**」（最终需求方）也会自动填入（定不下来时保持为空）。
4. 确认「**納品方法**」（送货方式）。需要时切换成「**通常納品**」（普通送货）或「**ユーザー直送**」（直送最终需求方）。
...
- 选择发货单时送货方式被自动填入的情况下，记载价格的初始值也按同样的规则决定。
...
作为依据的发货单。选择后会沿用该次发货的产品与数量。同时，送货方式、最终需求方、营业负责人在发货单一侧能各自定为一个时也会自动填入（各明细不一致、定不下来时不会填入）。
...
选择发货单后，若其订单回执上登记的配送方式能定为一种，会自动填入这里。
...
直送时的实际收货方。仅大客户会被登记。选择发货单时，收货公司能定为一家的话会自动填入（订单明细上有指定就用它，没有则用订单回执上登记的公司）。
