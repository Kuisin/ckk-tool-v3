...
When you ship, today's date is recorded as the **出荷日 (shipping date)** and the status becomes 「**出荷済**」 (Shipped). At the same time, the [product stock](/manual/en/operations/inventory/product-inventory/user) goes down by the pieces you sent, and the order line changes by itself to "一部出荷" (Partly shipped) or "出荷済" (Shipped).
...
A. The lot you chose does not have enough stock left for the pieces you want to send. Check the numbers in [product stock](/manual/en/operations/inventory/product-inventory/user) and try again.
