...
| 操作 | 会发生什么 |
|------|-----------|
| [应用开 / 关](#field-enabled) | 是否显示在主页与应用列表中 |

### 应用开 / 关 [#field-enabled]

关闭后，该应用会**从主页与应用列表中消失**。数据不会被删除 — 只是不再显示。

切换**按环境分别保存**（验证用与正式用相互独立）。请先在验证环境试用，再切换正式环境。
...
A. 开关只是「是否把应用显示在画面上」的设置。除此之外，那个人还必须拥有使用该应用的权限。请在 [用户管理](/manual/zh/operations/system/user-management/user) 中确认那个人的权限。
...
**Q. 想知道是谁在什么时候做了切换。**
A. 可以在 [操作历史](/manual/zh/operations/system/activity-log/user) 中确认。把「**対象**」（对象）筛选为「**アプリ管理**」（应用管理），就只会列出切换的记录。
