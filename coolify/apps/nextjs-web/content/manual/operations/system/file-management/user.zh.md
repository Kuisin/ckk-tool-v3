...
文件管理仅在设定访问权限时需要输入（仅限系统管理员）。其余操作只是切换显示。

| 项目 | 必填 | 填什么 |
|------|------|--------|
| [文件夹名称](#field-folder-name) | — | 表示文件保存位置的名称（仅供显示） |
...
表示文件保存位置的名称。在这个画面上无法新建文件夹 — 文件夹会在生成单据 PDF 或上传文件时自动产生。文件实际上并非存放于文件夹，而是**按命名方式分类**，因此文件夹名称即为保存位置的标识。点击画面上部「パス」（路径）一栏中的名称，可以回到该位置。
