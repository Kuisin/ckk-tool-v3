...
使用本画面需要 **二维码卡发放・PIN**（`kiosk_card`）权限。

| 想做的事 | 所需权限 |
| --- | --- |
| 打开画面、查看列表与明细 | 二维码卡发放・PIN — 查看 |
| 新增・变更・删除 | 二维码卡发放・PIN — 创建 / 更新 / 删除 |
...
| 操作 | 所需权限 | 可以做什么 |
| --- | --- | --- |
| 发放卡片 | 二维码卡发放・PIN（`kiosk_card`）— 创建 | 发放新的二维码卡。二维码的内容本身就是认证信息，印出的纸即为钥匙。 |
| 分配・改派卡片 | 二维码卡发放・PIN（`kiosk_card`）— 更新 | 将卡片绑定到员工。改派后，用该卡进行的操作将记在新的人名下。 |
| 注销卡片 | 二维码卡发放・PIN（`kiosk_card`）— 更新 | 使卡片不可用。丢失时这是正确的操作，且不可撤销。 |
| 重置 PIN | 二维码卡发放・PIN（`kiosk_card`）— 更新 | 将 PIN 恢复为未设定，由本人在下次登录时重新设定。由本人以外的人执行可能导致账户被冒用。 |
| 解除 PIN 锁定 | 二维码卡发放・PIN（`kiosk_card`）— 更新 | 立即解除因连续 5 次失败而产生的 15 分钟锁定。 |
| 变更有效期 | 二维码卡发放・PIN（`kiosk_card`）— 更新 | 变更卡片可用的期间。延长会使已过期的卡片复活。 |
| 变更同时会话上限 | 二维码卡发放・PIN（`kiosk_card`）— 更新 | 变更一张卡可同时开启的会话数。调高会使借用卡片变得难以发现。 |
| 输出卡纸 PDF | 二维码卡发放・PIN（`kiosk_card`）— 查看 | 将二维码输出为可打印的 PDF。下载之时，认证信息便以文件形式留在手边。 |
