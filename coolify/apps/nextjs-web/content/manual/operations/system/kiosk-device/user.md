...
| --- | --- | --- |
| メンテナンス退出 PIN の表示 | 共有端末の秘密（`kiosk_secret`）— 閲覧 | 全端末共通の退出 PIN を平文で表示する。これを知っている人はどの端末でも共有端末から抜けて Android 設定へ入れる |
| PIN 履歴の表示 | 共有端末の秘密（`kiosk_secret`）— 閲覧 | 過去 400 日ぶんの退出 PIN を一覧で表示する。オフラインの端末は古い PIN を保持しているため必要になるが、範囲は現行値より広い |
| 端末が保持している PIN の表示 | 共有端末の秘密（`kiosk_secret`）— 閲覧 | その端末に最後に渡した退出 PIN を表示する。オフラインの端末を開けるときに使う |
| 端末設定コードの表示 | 共有端末の秘密（`kiosk_secret`）— 閲覧 | その端末の設定画面（左上 5 タップ）を解錠するコードを表示する |
| 端末設定コードの再生成 | 共有端末の秘密（`kiosk_secret`）— 更新 | 設定コードを作り直す。現地に居る人が古いコードで入れなくなる |
| 端末鍵のリセット | 共有端末の秘密（`kiosk_secret`）— 更新 | アテステーション鍵の紐付けを外し、次に繋いだ端末を無条件に信頼し直す（TOFU）。端末を入れ替えたときだけ使う |
| 端末プロファイルの作成 | 端末アクセスの付与（`kiosk_device`）— 作成 | 新しい端末の枠を作る。ここにリンクした端末が現場でログイン画面を出せるようになる |
| 端末のリンク | 端末アクセスの付与（`kiosk_device`）— 更新 | 実機をプロファイルへ紐付ける。端末トークンが発行され、その端末がシステムに入る |
| 端末の有効化 | 端末アクセスの付与（`kiosk_device`）— 更新 | リンク済みの端末を稼働させる。これ以降その端末で従業員がログインできる |
| 端末の停止 / 再開 | 端末アクセスの付与（`kiosk_device`）— 更新 | 端末を一時的に止める、または止めていた端末を戻す |
| リンク解除 | 端末アクセスの付与（`kiosk_device`）— 更新 | 端末トークン・セッション・アテステーション鍵を破棄してプロファイルを空に戻す。名称と拠点は残る |
| 端末の失効 | 端末アクセスの付与（`kiosk_device`）— 更新 | その端末を即座に締め出す。現場で作業中のセッションも切れる |
| ディスプレイの登録 | 端末アクセスの付与（`kiosk_device`）— 作成 | 現場の画面を 1 枚増やす。登録した画面は誰でも見られる場所に業務データを映し続ける |
| ディスプレイの失効 | 端末アクセスの付与（`kiosk_device`）— 更新 | その画面を即座に締め出す。次の再読込で登録画面に戻り、以後は何も映らない |
