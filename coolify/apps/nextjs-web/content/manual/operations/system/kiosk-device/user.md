...
- **キオスク端末** … 工場に置いてある共有のタブレットのことです。この画面では「端末」と呼びます。
...
| --- | --- | --- |
| メンテナンス退出 PIN の表示 | キオスク端末の秘密（`kiosk_secret`）— 閲覧 | 全端末共通の退出 PIN を平文で表示する。これを知っている人はどの端末でもキオスクから抜けて Android 設定へ入れる |
| PIN 履歴の表示 | キオスク端末の秘密（`kiosk_secret`）— 閲覧 | 過去 400 日ぶんの退出 PIN を一覧で表示する。オフラインの端末は古い PIN を保持しているため必要になるが、範囲は現行値より広い |
| 端末が保持している PIN の表示 | キオスク端末の秘密（`kiosk_secret`）— 閲覧 | その端末に最後に渡した退出 PIN を表示する。オフラインの端末を開けるときに使う |
| 端末設定コードの表示 | キオスク端末の秘密（`kiosk_secret`）— 閲覧 | その端末の設定画面（左上 5 タップ）を解錠するコードを表示する |
| 端末設定コードの再生成 | キオスク端末の秘密（`kiosk_secret`）— 更新 | 設定コードを作り直す。現地に居る人が古いコードで入れなくなる |
| 端末鍵のリセット | キオスク端末の秘密（`kiosk_secret`）— 更新 | アテステーション鍵の紐付けを外し、次に繋いだ端末を無条件に信頼し直す（TOFU）。端末を入れ替えたときだけ使う |
