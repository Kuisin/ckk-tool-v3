...
本画面分为两部分：**产品项目**（字段本身）与**产品种别**（哪些产品显示哪些字段）。

### 产品项目的输入项

| 项目 | 必填 | 填什么 |
|------|------|--------|
| [项目名称（日语 / 英语）](#field-item-name) | 必填 | 产品画面上显示的字段名称 |
| [键（标识符）](#field-key) | 必填 | 保存时使用的英数字名称 |
| [类型](#field-type) | 必填 | 文本、数值等种类 |
| [默认值（基本）](#field-default) | 选填 | 未输入时采用的值 |
| [占位提示](#field-placeholder) | 选填 | 输入框中的浅色示例 |
| [设为必填项](#field-required) | — | 是否可以留空 |
| [模式（正则表达式）](#field-pattern) | 选填 | 文字形式的规则 |
| [最小值 / 最大值](#field-range) | 选填 | 数值范围 |

### 项目名称（日语 / 英语） [#field-item-name]

产品画面上显示的字段名称，需同时填写日语与英语。

### 键（标识符） [#field-key]

保存该字段时使用的英数字名称。**日后更改会导致此前输入的值无法关联**，请在登记时确定。

### 类型 [#field-type]

文本、数值等种类。类型决定**以下哪些设置生效** — 数值对应最小值・最大值，文本对应模式。

### 默认值（基本） [#field-default]

未输入时填入的值。设定常用值可减少输入。

### 占位提示 [#field-placeholder]

输入框中显示的浅色示例，不会作为值保存。

### 设为必填项 [#field-required]

开启后，该字段为空时将无法保存产品。

### 模式（正则表达式） [#field-pattern]
...
| 项目 | 必填 | 填什么 |
|------|------|--------|
| [种别名称（日语 / 英语）](#field-type-name) | 必填 | 创建产品时选择的名称 |
| [说明](#field-type-description) | 选填 | 适用于何种产品 |
| [有效](#field-type-active) | — | 是否出现在创建产品的选项中 |
| [项目](#field-type-items) | 选填 | 该种别使用的产品项目 |
| [默认值（覆盖）](#field-type-default) | 选填 | 仅该种别的默认值 |

### 种别名称（日语 / 英语） [#field-type-name]

创建产品时选择的名称。

### 说明 [#field-type-description]

关于该种别适用于何种产品的补充说明。

### 有效 [#field-type-active]

取消后，创建产品时的选项中将不再显示。**已使用该种别创建的产品保持不变。**

### 项目 [#field-type-items]

选择该种别时，产品画面上显示的字段，从上述产品项目中选取。

### 默认值（覆盖） [#field-type-default]

需要**仅在该种别下**使用不同默认值时填写。留空则采用产品项目本身的默认值。
