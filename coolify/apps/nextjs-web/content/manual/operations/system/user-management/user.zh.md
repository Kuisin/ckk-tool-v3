---
title: "用户管理 — 操作手册"
description: "用于查看系统使用者的一览、确认每个人可以做哪些事情，并变更其角色与所属据点的应用。"
...
无法在这里添加人员或修改姓名、邮箱（这些由公司内部的统一登录机制导入）。本画面可以变更的只有 **角色** 与 **所属据点** 两项。
...
- 有权限的人会看到「**編集**」（编辑）按钮。做法请参见「[变更角色](#变更角色)」。
...
这是这个人现在实际能做的事情的一览。无论持有多少角色，**每个许可只显示 1 行**，并列出该许可可执行的操作与所能达到的**最大范围**。
...
为某个人添加或去掉角色（ロール）。**变更角色会直接改变这个人能做的事情。**

只有拥有「变更用户管理」权限的人，才会在「**ロール割当**」右上方看到「**編集**」（编辑）按钮。

1. 打开想要变更的人的详细画面。
2. 点击「**ロール割当**」的「**編集**」。
3. 点击选择框，从一览中选择角色。可以选择多个。
4. 想要去掉的角色，再点击一次取消选择。
5. 点击「**保存**」（保存）；需要审批时点击「**承認を依頼**」（申请审批）。

系统管理员的变更会当场生效。**其他人则不会立即变更** — 填写「**変更の理由**」（变更理由）后提交，[特权访问](/manual/zh/operations/system/privileged-access/user)（SY0G）中会出现审批申请，**由其他人批准后变更才会生效**。不能自己申请再自己批准。

> ⚠️ 去掉全部角色后，这个人将无法进行任何操作。去掉之前请先确认他正在使用的画面。

> ⚠️ 当只有一名管理员时，无法从这个人身上去掉管理员角色，否则将无人能够恢复管理员。此时会显示「**最後の管理者から管理者ロールを外すことはできません**」（无法从最后一名管理员移除管理员角色）。

> 💡 自己的角色无法自己变更。请委托他人处理。

去掉的角色会作为记录保留。表中的「**解除日時**」（解除时间）会填入日期，之后可以查到是何时去掉的。

## 变更 所属拠点（所属据点）

只有拥有「变更用户管理」权限的人，才会在「**所属拠点**」右上方看到「**編集**」（编辑）按钮。
...
显示「**保存しました**」（已保存）「**所属拠点を更新しました**」（已更新所属据点）后即完成。与角色相同，系统管理员以外的人需填写理由后提交，经批准后变更才会生效。
...
用户的姓名与邮箱**由 Active Directory 导入**，无法在本画面更改。此处输入的仅为角色与所属基地。

| 项目 | 必填 | 填什么 |
|------|------|--------|
| [角色](#field-roles) | 选填 | 赋予该用户的角色 |
...
**这个人能做的事情，是所选全部角色加在一起的结果。** 结果可在同一画面的「実効権限」（实际生效权限）中确认。去掉全部角色后，这个人将无法进行任何操作。

仅拥有「变更用户管理」权限的人可编辑，系统管理员以外的人需要审批。自己的角色无法变更。
...
仅拥有「变更用户管理」权限的人可编辑，系统管理员以外的人需要审批。无权限的人只能以徽章形式查看当前所属。
...
**Q. 想变更某个人的角色（ロール）。**
A. 请在详细画面的「**ロール割当**」中点击「**編集**」（编辑）（参见「[变更角色](#变更角色)」）。若未出现按钮，说明没有权限，请委托系统管理员处理。

**Q. 已保存角色，但那个人的权限没有变化。**
A. 系统管理员以外的人所做的变更，**在获得批准之前不会生效**。请在[特权访问](/manual/zh/operations/system/privileged-access/user)（SY0G）中确认提交的申请是否仍在审批中。

**Q. 没有出现「編集」（编辑）按钮。**
A. 变更角色与所属据点需要「变更用户管理」权限。其他人只能查看。请委托系统管理员处理。

**Q. 已经给了角色，但那个人说打不开画面。**
A. 请在那个人的详细画面上查看「**実効権限**」（实际生效权限）中是否有对应的行。如果有行但数据是空的，请再确认「**所属拠点**」是否为空。

**Q. 持有 2 个角色的人，「実効権限」中同一个权限代码只显示了 1 行。**
A. 这是正常的。同一个许可**即使操作不同也合并为 1 行**。所有操作范围相同时只显示 1 个范围，仅当各操作范围不同时才分别标注。两个角色给出同样的许可时，按**较大的范围**显示（「仅本人负责」+「全公司」→「全公司」）。

**Q. 想知道是谁在什么时候变更了这个人的角色或所属据点。**
