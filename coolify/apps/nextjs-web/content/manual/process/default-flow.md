...
1. [納品書](/manual/ja/operations/shipping/delivery-note/user)（`SH02`）は**作らない** — 前段で出荷書を「確定」した時点で自動でできている。出荷書の「納品書」タブか納品書アプリの一覧から開き、届け先と価格記載を確かめる。直したいところがあれば、**発行する前（下書きのうち）**に「編集」で直す。
...
- 発行すると PDF が保存される。価格記載なしの納品書は価格の載らない書式になる。
