...
3. 指名された製造の担当者が「着手」を押し、図面ができたら添付して「完了」する。
...
- 完了すると、添付したファイルがまとめて 1 つの版（v1・v2 …）として登録され、[製品マスタ](/manual/ja/operations/masters/product/user)の最新図面が入れ替わる。指示書からその版を指定することもできる。
...
1. 顧客から届いた注文書（FAX・PDF）を[注文請書](/manual/ja/operations/sales/order-acceptance/user)（`SA04`）に取り込み、読み取り結果を確認・修正する。配送方法（通常配送・ユーザー直送）とエンドユーザー（最終需要家）もここで決める。**ユーザー直送でエンドユーザーが未指定のままだと、承認依頼も確定もできない**。
...
1. 出荷書から[納品書](/manual/ja/operations/shipping/delivery-note/user)（`SH02`）を作り、納品方法（通常納品・ユーザー直送）と価格記載の有無を決める。どちらも注文請書の配送方法から既定が自動で入り、最終需要家も引き継がれる。
...
- 出荷書を選ぶと、納品方法・価格記載・最終需要家の既定が注文請書の配送方法から自動で入る（ユーザー直送なら価格記載はオフ）。
