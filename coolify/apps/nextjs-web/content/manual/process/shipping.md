...
**納品書は作らない — 出荷書を確定した時点で自動でできている。** 何通できるかは出荷書に載っている明細の配送方法で決まる（束ねられる明細は全行が同じ配送方法なので、出荷書 1 通につき配送方法は 1 つに定まる）。
...
届け先（最終需要家）は注文明細のエンドユーザー欄から決まる（明細側でユーザー直送を選ぶとエンドユーザーは必須である）。1 つの出荷書に束ねられるのは**同じエンドユーザー**の注文明細だけで、これは自動作成が届け先を 1 件に決められるようにするための条件でもある。

できた納品書は**その時点で発行済**で、納品書側で内容を直す操作は無い（納品方法・最終需要家・価格記載・数量・単価はすべて注文明細と出荷書から引き継がれる）。間違いがあれば正しい注文請書・出荷書から作り直す。PDF はすぐに見られる。届いたら「納品済」にする。ここまでが済んだ出荷が、[請求の流れ](/manual/ja/process/billing)の対象になる（操作手順 … [標準フロー §10](/manual/ja/process/default-flow#stage-10)）。
...
**出したいロットが選べない**
その製品の在庫が、選んだ出荷元拠点に無い可能性がある。拠点を確認するか、[在庫管理](/manual/ja/operations/inventory/inventory-management/user)で在庫のある拠点を調べる。

**在庫が減らない**
出荷書が「確定」までで止まっていないかを確認する。在庫は出荷の記録で減る。

**納品書に金額を出したくない**
価格記載は出荷書に載っている明細の配送方法で決まり、納品書側では変えられない。ユーザー直送では、金額なしの 1 通が最終需要家宛に自動でできているので、そちらを渡す。通常配送の納品書は価格記載ありの 1 通だけになる。
