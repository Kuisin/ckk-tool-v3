...
- **右上角搜索框**（仅电脑） … 输入应用名称或“操作码”（如 `SA01`）并按回车即可直接跳转。手机等窄屏设备不显示该输入框 — 相同的搜索位于上方的应用列表（启动器）中。
...
每个界面都有 4 位编号。记住后可从右上角搜索框（电脑）或应用列表中的搜索快速跳转。
...
- **收到通知** … 可从右上角铃铛查看内容。
- **修改自己的设置** … 全部集中在右上角头像菜单中：“个人资料”可更改通知邮箱与密码，“主页设置”可更改收藏应用与显示方式，“显示设置”可更改界面语言、日期/时间格式、时区、文字大小与粗细（参见[用户设置手册](/manual/zh/user-settings)）。
