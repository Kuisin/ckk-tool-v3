...
**この画面は最初「見るだけ」で開きます。**「お気に入りアプリ」と「表示モード」のいまの設定が並んでいるので、変えるときは右上の「**編集**」を押してください。書き終わったら「保存」を押すと、また見るだけの表示に戻ります。
...
こちらも最初は「見るだけ」で開きます。右上の「**編集**」を押すと、下の項目とプレビューが出ます。
...
> 💡 **保存せずに画面を離れると、文字の大きさは元に戻ります。** 試してから決められます。

### 言語を変えると、画面の文字はすべて切り替わります

Web アプリと工場のタブレット（共有端末）の画面の文字は、**どちらも 3 言語ぶんそろっています**。日付・時刻・タイムゾーンの設定も、最初からすべての画面に効きます。

ただし**マスタに登録した文字は翻訳されません** — 製品名・取引先名・工程名・ロール名などは、登録されている言語のまま出ます（これらは各マスタ画面の「多言語」ボタンから言語ごとに登録できます）。
