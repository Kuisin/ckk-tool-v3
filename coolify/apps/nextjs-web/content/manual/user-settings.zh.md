---
title: "用户设置手册"
description: "与自己账号相关的设置。从右上角头像菜单 →“个人资料”/“通知设置”/“主页设置”/“显示设置”打开。"
screenshots: [profile-home-01, profile-preferences-01]
---
与自己账号相关的设置。从右上角头像菜单 →“个人资料”/“通知设置”/“主页设置”/“显示设置”打开。

> 💡 个人设置全部集中在该头像菜单中。主页上没有设置按钮。

## 个人资料

- **显示名称 / 用户名**：与公司目录（AD）同步。如需更改请联系信息系统部门。个人资料页面还会显示账号的类别与最后登录时间（所属部门仅显示在页头的头像菜单中）。
- **审批分组**：显示自己所属的审批分组。能否审批单据由此处显示的分组决定。如需变更分组请联系管理员。
- **邮箱（通知地址）**：接收通知的邮箱地址。若为空，则不发送邮件通知。

### 个人资料照片

- 在个人资料页面点击“设置照片”（已设置时为“更改”），可上传自己的照片。选择图片后会打开正方形裁剪界面，调整位置与大小后保存。
- 保存的照片会以大小两种尺寸存储；页头图标以及列表、历史记录中的小图标都使用同一张照片。
- 照片不会从公司目录（AD）获取，而是在本系统内设置。可用垃圾桶按钮删除，删除后恢复为姓名首字母显示。

## 密码

- 若使用 SSO 登录，密码由公司账号一侧管理。
- 如需更改系统内账号的密码，请在“个人资料 → 修改密码”中操作：输入当前密码与新密码（两次）。

## 通知

- **应用内通知**：显示在页头的铃铛处，可用“全部标记为已读”统一处理。
- **邮件通知**：发送至个人资料中的邮箱。
- **推送通知**：在“个人资料 → 通知设置”中按设备启用后，锁屏 / 桌面即可收到通知。
- **渠道开关**：“个人资料 → 通知设置”顶部有“邮件通知”“推送通知”的开 / 关开关。关闭邮件通知后，将不再收到审批请求等邮件。

## 按平台启用推送通知

- **Chrome / Edge（电脑）**：“个人资料 → 通知设置”→“在此设备上启用”，并在浏览器的通知权限提示中选择“允许”。
- **Android（Chrome）**：步骤相同。若出现“安装应用”按钮，安装后可从主屏幕启动（可选）。还需在系统设置中允许 Chrome 的通知。
- **iPhone / iPad（iOS 16.4 及以上）**：无法在 Safari 标签页中启用。请使用 Safari 的分享按钮 →“添加到主屏幕”，从主屏幕的“CKK”图标重新打开后，再点击“在此设备上启用”。
- 若误将通知设为拦截，请在浏览器的网站设置中将通知改回“允许”后再重新启用。

## 已注册设备

- 在“个人资料 → 通知设置”的“已注册设备”中，可查看并删除已启用推送通知的设备。请删除不再使用的终端。

## 主页设置

通过头像菜单中的“主页设置”，可自定义主页的排列。

**本画面默认以“仅查看”方式打开。** 上面列出当前的收藏应用与显示模式，要修改请点击右上角的「**編集**」（编辑）。保存后会回到仅查看的显示。

- **收藏应用** … 点击卡片选择应用后，它们会作为“收藏”集中显示在主页最上方，显示顺序即选择顺序。
- **显示模式** … 可在“标准（按类别）”与“自定义（按分组）”之间切换。自定义模式下按自建分组排列应用，未加入任何分组的应用会集中在“其他”。
- **自定义分组** … 在“新分组”中输入分组名并添加。每个分组可以重命名、上移 / 下移、删除，并可选择所属应用（一个应用最多只能属于一个分组；已在其他分组中的应用无法选择）。

![主页设置](./assets/screenshots/profile-home-01.png)

## 显示设置

通过头像菜单中的“**显示设置**”，可以更改界面语言、日期时间的显示方式以及文字大小。设置**仅适用于本人**（语言与日期时间在网页端与车间平板（kiosk）通用，文字大小与加粗仅限网页端）。

本画面同样默认以“仅查看”方式打开。点击右上角的「**編集**」（编辑）后，才会出现下面的项目与预览。

- **语言** … 可选择 日本語 / English / 中文。
- **日期格式** … 可选择 `2026/03/05`、`2026-03-05`、`05/03/2026`、`03/05/2026`。
- **时间格式** … 24 小时制（14:30）或 12 小时制（2:30 PM）。
- **时区** … 以哪个地区的时间显示日期时间。在海外厂区或出差时使用可据此调整。选项中会附带该地区的**当前时间**，一眼即可看出时差。
- **文字大小** … 最小 / 小 / 标准 / 大 / 最大 共 5 档。中间的“标准”即为此前的大小，更改后**整个界面**的文字与间距会一同变大（变小）。每个选项均以其所代表的大小书写，按下之前即可预估效果。
- **加粗文字** … 将正文加粗一档。原本就加粗的标题与强调保持不变。

选择后会立即反映到下方的“**预览**”，保存前即可确认效果。**文字大小与粗细会立即应用到整个界面**（是否易读要连同列表与按钮一起判断，小小的样例看不出来）。保存后会应用到**所有显示日期时间的位置**（列表、详情、历史等）。

![显示设置](./assets/screenshots/profile-preferences-01.png)

> 💡 **更改时区不会改变已记录的时间本身。** 只是改为以哪个地区的时钟来读取同一事件。

> 💡 **未保存就离开页面，文字大小会恢复原样**，可以先试再决定。

### 切换语言后，画面文字会全部切换

网页端与车间平板（kiosk）的界面文字**三种语言均已齐备**。日期、时间、时区设置也对所有画面生效。

但是**主数据中登记的文字不会翻译** — 产品名称、业务伙伴名称、工序名称、角色名称等，会按登记时的语言显示（这些可以在各主数据画面的「多言語」（多语言）按钮中按语言登记）。

### 单据（PDF）不遵循此设置

报价单、送货单、发票等 PDF **始终以日本时间和统一格式**打印，以免同一份已完成的单据因查看者不同而显示不同日期。

## 手册语言

- 手册（/manual）可通过右上角的语言链接在 日本語 / English / 中文 之间切换。应用本身的语言请在上方的“**显示设置**”中更改。
