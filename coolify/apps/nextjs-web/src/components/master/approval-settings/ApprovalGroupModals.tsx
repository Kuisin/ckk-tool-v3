  ...
  return (
    <ConfirmModal
      ...
      message={
        target
          ? tr(
              "承認グループ「{name}」を削除します。メンバー割当も同時に削除されます。この操作は取り消せません。",
              { name: target.name },
            )
          : ""
      }
      ...
      onConfirm={() => {
        ...
        startTransition(async () => {
          ...
          if (result.ok) {
            notifications.show({
              title: tr("削除しました"),
              message: tr("承認グループ「{name}」を削除しました", {
                name: target.name,
              }),
              color: "green",
            });
    ...
    <ConfirmModal
      ...
      message={
        target
          ? isActive
            ? tr(
                "承認グループ「{name}」を無効化します。新規の承認依頼で使用できなくなります。",
                { name: target.name },
              )
            : tr(
                "承認グループ「{name}」を有効化します。再び承認依頼で使用できるようになります。",
                { name: target.name },
              )
          : ""
      }
      ...
      onConfirm={() => {
        ...
        startTransition(async () => {
          ...
          if (result.ok) {
            notifications.show({
              title: tr("追加しました"),
              message: tr("メンバー「{userLabel}」を追加しました", {
                userLabel: userLabel,
              }),
              color: "green",
            });
      ...
      onConfirm={() => {
        ...
        startTransition(async () => {
          ...
          if (result.ok) {
            notifications.show({
              title: tr("保存しました"),
              message: tr("「{displayName}」の在籍期間を更新しました", {
      ...
      onConfirm={() => {
        ...
        startTransition(async () => {
          ...
          if (result.ok) {
            notifications.show({
              title: tr("追加しました"),
              message: tr("代理人「{delegateLabel}」の代理設定を追加しました", {
  ...
  return (
    <ConfirmModal
      ...
      message={
        delegate
          ? tr(
              "代理設定（原承認者「{delegatorName}」→ 代理人「{delegateName}」）を削除します。代理人はこのグループの承認を行えなくなります。",
              {
                delegatorName: delegate.delegatorName,
      ...
      onConfirm={() => {
        ...
        startTransition(async () => {
          ...
          if (result.ok) {
            notifications.show({
              title: tr("削除しました"),
              message: tr("代理人「{delegateName}」の代理設定を削除しました", {
                delegateName: delegate.delegateName,
              }),
              color: "green",
            });
  ...
  return (
    <ConfirmModal
      ...
      message={
        member
          ? tr(
              "メンバー「{displayName}（{username}）」をこのグループから削除します。",
      ...
      onConfirm={() => {
        ...
        startTransition(async () => {
          ...
          if (result.ok) {
            notifications.show({
              title: tr("削除しました"),
              message: tr("メンバー「{displayName}」を削除しました", {
