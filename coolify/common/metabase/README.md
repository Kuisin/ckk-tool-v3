...
- **業務（db 5）**: `sql/gen-business-ja.py` が対応表を持ち、出力
  ...
  JSON 列（監査差分・価格試算結果など）は展開を切って項目一覧を汚さない。列を
  追加したら**生成元の .py を直して**再生成する:

```bash
# 労務ラベル
ssh 192.168.50.15 "docker exec -i metabase-db psql -U metabase -d metabase -v ON_ERROR_STOP=1" \
  < coolify/common/metabase/sql/metadata-ja.sql
# 業務ラベル（生成 → 適用）
python3 coolify/common/metabase/sql/gen-business-ja.py > coolify/common/metabase/sql/metabase-business-ja.sql
ssh 192.168.50.15 "docker exec -i metabase-db psql -U metabase -d metabase -v ON_ERROR_STOP=1" \
  < coolify/common/metabase/sql/metabase-business-ja.sql
...
- 金額系ビューは末尾に**通貨換算列**を持つ: `currency`（書類の通貨、既定 JPY）と
  `*_jpy` / `*_usd`（`app.currencies` の `rate_per_100_jpy` = 100 円で買える
  その通貨量、JPY = 100 基準。`v_currencies` で参照できる。分析用換算 —
  会計処理用ではない）。
...
生産進捗 / 請求 / 在庫** の 4 ダッシュボード + 27 カードを作る。カードは
**標準 UI エディタ（クエリビルダー / MBQL）の質問**として作る — 利用者が
ノートブックエディタで開いて編集できる（native SQL は使わない方針。ソースは
analytics ビュー）。列見出しはビューの日本語表示名がそのまま出る。状態 enum の
日本語表示は **field value remapping**（ビルダーが自動設定 — 表・グラフ・
フィルタのドロップダウンすべて日本語）。全ダッシュボードに「期間」「通貨」
フィルタ付き（MBQL なので列へ直接マップ）。名前で冪等 — 直したら再実行すれば
作り直さず更新する。
...
MB_URL=https://bi.ckk-tool.co.jp MB_API_KEY=mb_<admin> MB_DB_ID=5 MB_COLLECTION_ID=6 \
  python3 coolify/common/metabase/build-business-dashboards.py
```
