...
(`dockerfile`, base dir `/coolify/apps/admintools`), auto-deploying from GitHub.
...
dockerfile `/coolify/apps/nextjs-kiosk/Dockerfile`), registered by
...
`package.json`); admintools keeps `coolify/apps/admintools/**`. So a docs-only
...
`PATCH /api/v1/applications/<uuid> {"watch_paths": "coolify/apps/admintools/**"}`.
