...
| `999_audit_backfill.sql.gz` | 監査ログ backfill: 上記で投入した現行マスタ（取引先/材種/素材/製品）各行に SEED 監査行（actor=システム）を生成し、詳細「履歴」タブ・操作履歴一覧を実データに沿わせる。生きているテーブルから SELECT するため mapped.sqlite 不要。record_id は各画面が参照するキー（BP=uuid, 他=内部 id::text）に一致。~48,000 行（うち製品 ~43,444） |
