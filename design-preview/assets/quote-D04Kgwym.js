...
<body>
  ...
  <div class="meta-row">
    ...
    <div class="doc-info">
      <table>
        <tr><td>見積書番号</td><td>{{doc.number}}</td></tr>
        <tr><td>発行日</td><td>{{doc.issued_date}}</td></tr>
        <tr><td>有効期限</td><td>{{doc.valid_until}}</td></tr>
  ...
  <div class="strip">
    本見積書の有効期限は <strong>{{doc.valid_until}}</strong> までとなります。期限内にご注文をお願いいたします。
  </div>
  ...
  <div class="totals">
    <table>
      ...
      <tr class="grand-total"><td>合計金額（税込）</td><td>¥ {{totals.grand_total}}</td></tr>
    </table>
  </div>
