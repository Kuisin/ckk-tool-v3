...
function makeResource(
  ...
  const make = (mode: string, kind: OperationCodeKind, labelSuffix: string, href: string): OperationCodeEntry => ({
...
export const OPERATION_CODES: OperationCodeEntry[] = [
  ...
  ...makeResource('販売', 'SA', '5', '試算',         '/sales/estimates'),
  ...
  ...makeResource('生産', 'PD', '4', '製品在庫',   '/production/inventory/products'),
  ...makeResource('生産', 'PD', '5', '素材在庫',   '/production/inventory/materials'),
  ...
  ...makeResource('マスタ', 'MS', '8', '検査表テンプレート',   '/master/inspection-templates'),
  ...makeResource('マスタ', 'MS', '9', '不良種類',             '/master/defect-types'),
  ...makeResource('マスタ', 'MS', 'A', '承認グループ',         '/master/approval-groups'),
];
...
const MAX_INPUT_LENGTH = OPERATION_CODE_LENGTH + OPERATION_CODE_UPDATE_SUFFIX.length;
...
export function sanitizeOperationCodeInput(input: string): string {
  const upper = input.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, MAX_INPUT_LENGTH);
  if (upper.length > OPERATION_CODE_LENGTH && !upper.endsWith(OPERATION_CODE_UPDATE_SUFFIX)) {
...
export function isValidOperationCodeFormat(code: string): boolean {
  return (
    (code.length === OPERATION_CODE_LENGTH || code.length === MAX_INPUT_LENGTH)
...
function parseCode(code: string): { categoryCode: string; mode: string; index: string } | null {
  const upper = code.toUpperCase();
  if (upper.length !== OPERATION_CODE_LENGTH && upper.length !== MAX_INPUT_LENGTH) return null;
...
export function getListOperationCode(href: string): string | null {
  const entry = OPERATION_CODES.find((e) => e.kind === 'list' && e.href === href);
  return entry?.code ?? null;
}

/** コード・ラベルで操作コードを検索（オートコンプリート用） */
export function searchOperationCodes(query: string, limit = 8): OperationCodeEntry[] {
  const q = query.trim();
  if (!q) return [];
  ...
  return OPERATION_CODES.filter((entry) => {
    ...
    if (entry.category !== '共通' && entry.category.toLowerCase().includes(lower)) return true;
    return false;
  }).slice(0, limit);
}
