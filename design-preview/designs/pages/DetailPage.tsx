/**
 * DetailPage.tsx — Sample: 指示書詳細 (Work Order Detail)
 *
 * This file shows the standard DETAIL PAGE pattern used for all [id]/page.tsx routes.
 * The same structure applies to: sales orders, invoices, shipping orders, etc.
 *
 * ─── COMPONENT MAP ───────────────────────────────────────────────────────────
 *
 *  Stack (gap="md")
 *  ├── Group (page header)
 *  │   ├── Stack > Breadcrumbs + Group(Title + StatusBadge)
 *  │   └── Group (action buttons)
 *  │       ├── Button edit
 *  │       ├── Button PDF
 *  │       └── Menu (more actions)
 *  ├── SimpleGrid (summary card — FieldValue pairs)
 *  ├── ApprovalStatusPanel (if approval_status != NONE)    [Custom]
 *  ├── WorkOrderStepsPanel (process step workflow)         [Custom]
 *  └── Tabs
 *      ├── Tabs.Panel "明細" — line items Table
 *      ├── Tabs.Panel "関連" — related documents list
 *      └── Tabs.Panel "履歴" — AuditTimeline
 *
 * ─── CUSTOMIZATIONS ──────────────────────────────────────────────────────────
 *
 * [Mantine default] Stack, Group, SimpleGrid, Tabs, Breadcrumbs, Title, Paper,
 *   Button, Menu, Table, Text, Badge — all standard Mantine, no customization.
 *
 * [Custom] StatusBadge: displayed inline next to the title (not as a separate row).
 *   The Title + StatusBadge pair is a standard pattern across all detail pages.
 *
 * [Custom] FieldValue component: renders a label+value pair in the summary grid.
 *   Simple but extracted to a component because it's used on every detail page.
 *   Defined in src/components/ui/FieldValue.tsx.
 *
 * [Custom] ApprovalStatusPanel: shown only when work_order.approval_status != NONE.
 *   Uses Mantine Stepper to visualize the 2-step approval flow.
 *   Includes "承認" / "差し戻し" buttons for eligible approvers.
 *
 * [Custom] WorkOrderStepsPanel: renders the manufacturing workflow as StepCard components.
 *   Each StepCard shows: step name, status icon (color-coded), timestamps, supplier (if outsource).
 *   Status icons use ThemeIcon with color determined by STEP_STATUS enum.
 *
 * [Custom] AuditTimeline (in 履歴 tab): renders audit_logs in reverse-chronological order.
 *   Uses Mantine Timeline component. Each item shows user avatar initial as bullet.
 *
 * [Mantine] Tabs defaultValue selects the first tab. Tab titles are Japanese labels.
 *   In production, the active tab can also be synced to a URL search param
 *   (?tab=items) to preserve state across navigation.
 *
 * [Mantine] Menu for "more actions" (cancel, copy) — standard overflow pattern.
