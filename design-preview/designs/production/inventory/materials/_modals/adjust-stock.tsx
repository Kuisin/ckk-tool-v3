/**
 * adjust-stock.tsx — 素材在庫 棚卸調整ポップアップ（inventory_transactions: ADJUST）
 *
 * Controlled FormModal: 調整後数量 と 理由 を入力して素材在庫を補正する。
...
export function AdjustMaterialStockModal({
  opened,
  onClose,
  label,
  unit,
  ...
  const handleSubmit = (values: Values) => {
    startTransition(async () => {
      console.log('Adjust material stock:', values);
      notifications.show({ title: '保存しました', message: '素材在庫を調整しました', color: 'green' });
      form.reset();
      onClose();
    });
  };
  ...
  return (
    ...
    >
      ...
      <NumberInput label="調整後の在庫数" withAsterisk min={0} decimalScale={3} suffix={` ${unit}`} {...form.getInputProps('quantity')} />
