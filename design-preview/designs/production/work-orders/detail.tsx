...
import { CancelWorkOrderModal } from './_modals/cancel';
import { CopyVersionWarningModal } from './_modals/copy-version-warning';
import { RequestChangeApprovalModal } from './_modals/request-change-approval';
...
export default function WorkOrderDetailPage() {
  ...
  const canRequestChange = WO.status === 'APPROVED' || WO.status === 'IN_PROGRESS';
  ...
  return (
    <DetailShell
      ...
      createdAt={`${formatDateTime(WO.createdAt)}（${WO.createdBy}）`}
    ...
    >
              ...
              <Group gap="sm" wrap="nowrap" align="flex-start">
                ...
                <Stack gap={2}>
                  <Group gap="xs">
                    ...
                    <Badge size="xs" color="green">承認</Badge>
                  </Group>
      ...
      {/* ── WorkOrderStepsPanel (design.md §12.2) — bespoke, kept inline ── */}
      <Paper withBorder p="md" radius="md">
        <Group justify="space-between" mb="sm">
          <Title order={5}>工程ワークフロー</Title>
          {!isMobile && canRequestChange && (
            <Button variant="subtle" size="xs" onClick={() => setChangeApprovalOpen(true)}>
              変更承認依頼
            </Button>
          )}
        </Group>
            ...
            <Paper key={step.id} withBorder p="sm" radius="sm">
              ...
              {step.location === 'OUTSOURCE' && (
                <Group gap="xl" mt="xs" pl={28}>
                  <Text size="xs" c="dimmed">依頼: {formatDate(step.requestedAt)}</Text>
                  <Text size="xs" c="dimmed">入荷予定: {formatDate(step.expectedAt)}</Text>
                </Group>
              )}
              {step.status === 'COMPLETED' && step.completedAt && (
                <Group gap="xl" mt="xs" pl={28}>
                  <Text size="xs" c="dimmed">完了: {formatDateTime(step.completedAt)}（{step.completedBy}）</Text>
                </Group>
              )}
            </Paper>
          ))}
        ...
        {isMobile && canRequestChange && (
          <Button variant="subtle" size="xs" mt="sm" fullWidth onClick={() => setChangeApprovalOpen(true)}>
            変更承認依頼
          </Button>
        )}
      </Paper>
      ...
      <Tabs defaultValue="items">
        ...
        <Tabs.Panel value="related" pt="md">
          <Stack gap="sm">
            ...
            <Divider />
            <Group>
              <Text size="sm" c="dimmed" w={120}>検査表</Text>
              <Stack gap={2}>
                <Text size="sm" c="blue">円筒加工検査表</Text>
                <Text size="sm" c="blue">出荷前検査表</Text>
              </Stack>
            </Group>
          </Stack>
        </Tabs.Panel>
      ...
      <CancelWorkOrderModal opened={cancelOpen} onClose={() => setCancelOpen(false)} workOrderNumber={WO.workOrderNumber} />
      <CopyVersionWarningModal opened={copyOpen} onClose={() => setCopyOpen(false)} sourceLabel={`指示書 #${WO.workOrderNumber}`} />
      <RequestChangeApprovalModal opened={changeApprovalOpen} onClose={() => setChangeApprovalOpen(false)} workOrderNumber={WO.workOrderNumber} />
    </DetailShell>
  );
}
