    ...
    >
      ...
      <FormSection title="コピー作成">
        ...
        {form.values.sourceWorkOrderId && (
          <Alert color="yellow" icon={<IconInfoCircle size={16} />} mt="sm" title="バージョン警告">
            コピー元の指示書（{SOURCE_WO_OPTIONS.find((o) => o.value === form.values.sourceWorkOrderId)?.label}）は作成後に製品仕様・工程マスタが更新されている可能性があります。工程ワークフローを必ず確認してください。
          </Alert>
        )}
      </FormSection>
      ...
      <FormSection title="工程ワークフロー">
        ...
        <Alert color="blue" icon={<IconInfoCircle size={16} />} mb="md" title="工程依存チェック">
          <Stack gap={4}>
            <Text size="sm">円筒加工には<b>円筒加工検査・円筒加工検査承認</b>が必要です（使用依存）。</Text>
            <Text size="sm">各工程は前工程の完了後に開始可能になります（実行依存）。順序は実行時に依存解決で決定されます。</Text>
          </Stack>
        </Alert>
