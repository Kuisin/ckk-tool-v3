...
export default function QuoteEditPage() {
  ...
  const form = useForm<QuoteFormValues>({
    ...
    initialValues: {
      ...
      notes: '納期厳守でお願いします。',
      items: [
        { productId: 'PRD-2601-0001', orderType: 'PRODUCTION', quantity: 50, unitPrice: 5000, deliveryDate: new Date('2026-07-10'), notes: '' },
        { productId: 'PRD-2602-0008', orderType: 'TEST', quantity: 5, unitPrice: 6200, deliveryDate: null, notes: '' },
      ],
    },
  });
  ...
  const handleSubmit = (values: QuoteFormValues) => {
    startTransition(async () => {
      try {
        ...
        notifications.show({ title: '保存しました', message: '見積書を更新しました', color: 'green' });
      } catch {
  ...
  return (
    <Stack gap="md">
      <PageHeader
        breadcrumbs={['ホーム', '販売', '見積書', '編集']}
        title="見積書 編集"
        status={<StatusBadge entity="Quote" status="DRAFT" />}
      />
