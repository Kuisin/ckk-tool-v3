...
# extractor/Dockerfile は自分のディレクトリを build context として書かれている
# （ai-stack compose の `build: ./extractor` と同じ）。
BASE_DIR="/docker-compose/ai-stack/extractor"
# dockerfile_location は base_directory からの相対（Coolify が連結する）。
# 絶対パスで書くと /…/extractor/docker-compose/…/Dockerfile を探して失敗する。
DOCKERFILE="/Dockerfile"
WATCH_PATHS="docker-compose/ai-stack/extractor/**"
...
次の手順:
  1. ai-stack をデプロイして ollama を coolify ネットワークへ参加させる
     （docker-compose/deploy-stack.sh ai-stack）— これが無いと抽出できない。
