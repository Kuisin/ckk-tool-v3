#!/usr/bin/env python3
"""Metabase「CKK 業務」データソース（app スキーマ）の表示名を日本語化する SQL を生成する。

生の DB 名の自動整形（Order Acceptances / Unit Price …）ではなく、意味の分かる
日本語ラベルに揃える。テーブル名・列名の対応表がこのファイルの唯一の正で、
出力 `metabase-business-ja.sql`（冪等）を Metabase アプリ DB に適用する。

  python3 gen-business-ja.py > metabase-business-ja.sql

対応は _specs/tables.md / _specs/design.md の用語集に準拠。列名はこの DB 内で
概ね同義（created_at はどこでも「作成日時」）なので列名だけをキーに全テーブルへ
当て、意味が分かれる少数の列はテーブル横断でも通じる語を選ぶ。
"""

# ─── テーブル表示名（app スキーマ 108 表） ───────────────────────────
TABLES = {
    # 承認
...
# ─── 列表示名 ────────────────────────────────────────────────────────
# 列名 → 意味の日本語。この DB では同名列はほぼ同義なので列名だけをキーに
# 全テーブルへ当てる。金額列は「(税抜)」等は付けず素直な語にする。
COLS = {
    # 主キー・共通メタ
    "id": "ID",
    "created_at": "作成日時",
...
# ─── フリーフォーム JSON 列（BI で展開しない） ──────────────────────
# Metabase は JSON 列を子フィールドへ自動展開する。監査差分や試算結果のような
# 構造化ブロブは展開すると項目一覧を汚すだけなので unfolding を切る。
# 一方 { ja, en } 多言語列は ja/en の 2 葉だけなので展開を残し、葉に和名を付ける。
BLOB_JSON = [
    "after_data", "before_data", "input", "result", "spec", "history",
    "payload", "flow_snapshot", "options", "settings", "content",
    "measured_values", "defect_reasons",
]
...
def main() -> None:
    out = []
    out.append("-- 生成物: gen-business-ja.py が出力。手で編集しない（生成元を直す）。")
    out.append("-- Metabase「CKK 業務」データソース（app スキーマ）の表示名を日本語化する。")
    out.append("-- 適用先は Metabase アプリ DB（metabase-db）。冪等。")
    out.append("--   ssh 192.168.50.15 \"docker exec -i metabase-db psql -U metabase -d metabase\" < metabase-business-ja.sql")
    out.append("")
    out.append("BEGIN;")
    out.append("")
    out.append("-- ─── テーブル表示名 ─────────────────────────────")
    ...
    rows = [f"  ({sql_lit(k)}, {sql_lit(v)})" for k, v in TABLES.items()]
    out.append(",\n".join(rows))
    out.append(")")
    out.append("UPDATE metabase_table t SET display_name = m.ja")
    out.append("FROM m, target")
    out.append("WHERE t.db_id = target.id AND t.schema = 'app' AND t.name = m.tbl")
    out.append("  AND t.display_name IS DISTINCT FROM m.ja;")
    out.append("")
    out.append("-- ─── 列表示名 ───────────────────────────────────")
    out.append("WITH target AS (SELECT id FROM metabase_database WHERE name = 'CKK 業務'),")
    out.append("m(col, ja) AS (VALUES")
    rows = [f"  ({sql_lit(k)}, {sql_lit(v)})" for k, v in COLS.items()]
    out.append(",\n".join(rows))
    out.append(")")
    ...
    out.append("")
    out.append("-- ─── 多言語 { ja, en } 列の葉に和名 ────────────────")
    out.append("WITH target AS (SELECT id FROM metabase_database WHERE name = 'CKK 業務')")
    out.append("UPDATE metabase_field f SET display_name = CASE f.name WHEN 'ja' THEN '日本語' ELSE '英語' END")
    out.append("FROM metabase_table t, metabase_field p, target")
    out.append("WHERE f.table_id = t.id AND t.db_id = target.id AND t.schema = 'app'")
    out.append("  AND f.parent_id = p.id AND p.name NOT IN (" + ", ".join(sql_lit(b) for b in BLOB_JSON) + ")")
    out.append("  AND f.name IN ('ja','en')")
    out.append("  AND f.display_name IS DISTINCT FROM (CASE f.name WHEN 'ja' THEN '日本語' ELSE '英語' END);")
    out.append("")
    out.append("-- ─── フリーフォーム JSON 列は展開しない ────────────")
    out.append("WITH target AS (SELECT id FROM metabase_database WHERE name = 'CKK 業務')")
    out.append("UPDATE metabase_field f SET json_unfolding = false")
    out.append("FROM metabase_table t, target")
    out.append("WHERE f.table_id = t.id AND t.db_id = target.id AND t.schema = 'app'")
