...
| `https://kiosk.ckk-tools.loc` | **生产** | 公司内部专用地址（自签名证书） |

> 平板终端**保持使用 `kai-lab.net`**，不迁移到 `ckk-tool.co.jp`。
> 因为它只在车间平板上使用，今后计划改为**仅限公司内部网络**访问。
> 终端侧无需任何变更。
>
> `kiosk.ckk-tools.loc` 就是该内部专用地址。`.loc` 不是公开域名，因此证书为
> **自签名**，需要满足两个前提：在公司内部 DNS 中添加
> `kiosk.ckk-tools.loc → 192.168.50.15`；以及**让平板应用信任该证书**
> （目前应用会拒绝自签名证书 —— 参见下方注意事项）。
> 通过浏览器访问时，确认安全警告后即可打开。
...
- **目前无法从平板终端打开 `kiosk.ckk-tools.loc`。** Android 版终端应用会拒绝
  自签名证书（未配置 `network_security_config`，也未处理 SSL 错误），
  因此若要在平板上使用，需要修改应用以信任该证书并重新构建，或引入公司内部 CA。
