...
This app performs the once-a-month "closing". It aggregates the target month's uninvoiced shipments (**shipped × dispatch** [shipping orders](/manual/en/apps/shipping-order/user)) per customer into closing rows, then generates draft [invoices](/manual/en/apps/invoice/user) from them.
...
- Each customer's closing day comes from the **締日** (closing day) setting in the [customer](/manual/en/masters/customer/user) master (month-end when not set).
...
4. Subsequent issuing, sending, payment recording, and Yayoi accounting CSV export are done in the [invoice](/manual/en/apps/invoice/user) app.
