...
---
Once a month, this app gathers "everything sent that month" for each customer and prepares the basis for their [invoice](/manual/en/apps/invoice/user). The operation code is `BL02`.
...
- Make a draft [invoice](/manual/en/apps/invoice/user) with one button.
...
- Only [shipping orders](/manual/en/apps/shipping-order/user) that are **「出荷済」 (Shipped) and of type 「発送」 (Dispatch)** are added up. Shipping orders still at 「下書き」 (Draft) or 「確定」 (Confirmed) are not included. Please record the shipment first.
- Each customer's cut-off date comes from the registered details of the [customer](/manual/en/masters/customer/user). Customers with nothing set are gathered as **month-end**.
...
- Click the blue shipping order number to open that [shipping order](/manual/en/apps/shipping-order/user) and check its contents.
...
A draft invoice is made, and you go straight to the [invoice](/manual/en/apps/invoice/user) screen. The consumption tax and the payment due date are filled in for you from the customer's registered details.
...
After this, recording the issue, the sending, and the payment — and making the file for the accounting software — is done in the [invoice](/manual/en/apps/invoice/user) app.
...
A. Each customer's cut-off date comes from the registered details of the [customer](/manual/en/masters/customer/user). Customers with nothing set become month-end. Fix the registered details and then run the monthly billing close again.
