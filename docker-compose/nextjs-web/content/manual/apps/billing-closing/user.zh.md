...
---
每月一次，按客户把「当月发出的部分」汇总起来，做出[发票](/manual/zh/apps/invoice/user)基础的应用。操作代码是 `BL02`。
...
- 一个按钮就能做出[发票](/manual/zh/apps/invoice/user)的草稿。
...
- 被汇总的，只有 **状态是「出荷済」（已发货）、类别是「発送」（发送）的**[出货单](/manual/zh/apps/shipping-order/user)。还停在「下書き」（草稿）「確定」（确定）的出货单不会算进去。请先完成发货记录。
- 每个客户的结算截止日，由[客户](/manual/zh/masters/customer/user)的登记内容决定。没有设定的客户，按 **月末** 汇总。
...
- 点出货单号的蓝色文字，可以打开该[出货单](/manual/zh/apps/shipping-order/user)确认内容。
- 表格最下面会显示数量和金额的合计。
...
发票的草稿会被做出来，并直接跳到[发票](/manual/zh/apps/invoice/user)界面。消费税和付款期限，会根据客户的登记内容自动填入。

结算的行会变成「**処理済**」（已处理），从「**生成請求書**」（生成的发票）栏里，随时都能打开那张发票。

![变成已处理的结算截止处理](../../assets/screenshots/billing-closing-processed-01.png)

之后的发行、寄送、收款记录，以及做出会计软件用的文件，都在[发票](/manual/zh/apps/invoice/user)应用里进行。
...
**Q. 只有某个客户的结算截止日不对。**
A. 每个客户的结算截止日，由[客户](/manual/zh/masters/customer/user)的登记内容决定。没有设定的客户会按月末处理。请修改登记内容后，再执行一次结算截止处理。
