...
- 指定了产品时，完成后[产品主数据](/manual/zh/masters/product/user)的最新图纸会随之更换。
...
- 如果图纸对象的[产品](/manual/zh/masters/product/user)已经登记，完成时就能作为该产品的最新图纸保存下来（不选产品也可以提出委托）。
- 用「見積時」（报价时）登记时，先有[报价单](/manual/zh/apps/quote/user)就能和该报价关联起来。
- 用「受注時」（接单时）登记时，先有注文請書（销售订单）就能关联起来（销售订单由[订单受理](/manual/zh/apps/order-acceptance/user)的「伝票展開」（单据展开）生成）。
...
4. 选了接单时的话，在「**注文請書**」（销售订单）栏里搜索并选择作为依据的销售订单（留空也能登记）。
...
> ⚠️ 「トリガー」（触发），以及在那里选的报价单、销售订单 **之后不能更改**。弄错时请重新建立一份新的委托。
...
**Q. 触发或报价单栏改不了。**
A. 触发，以及在那里选的报价单、销售订单，建立之后就不能更改。弄错时请重新建立一份新的委托，弄错的那份可以就这样放着，或与负责人商量。
