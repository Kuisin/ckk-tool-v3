...
There is no "New" button in this app. Invoices are made for you by [monthly billing close](/manual/en/apps/billing-closing/user).
...
- **由来 (source)** … Shows which [shipping order](/manual/en/apps/shipping-order/user) or [delivery note](/manual/en/apps/delivery-note/user) a line came from.

## Before you start

- You cannot make an invoice in this app. First use [monthly billing close](/manual/en/apps/billing-closing/user) and press "請求書を生成" (Generate invoice).
...
- **由来 (Source)** … Click the blue text to open the [shipping order](/manual/en/apps/shipping-order/user) or [delivery note](/manual/en/apps/delivery-note/user) it came from.
...
A. Invoices are not made on this screen. Open [monthly billing close](/manual/en/apps/billing-closing/user) and press "請求書を生成" (Generate invoice) on the row you want.

**Q. I want to fix an amount, but there is no edit screen.**
A. Invoices cannot be fixed directly. Something is probably wrong on the original [shipping order](/manual/en/apps/shipping-order/user), so please talk to the accounting or sales person in charge.
...
A. The tax is worked out with the rule set for each customer. Customers on the reduced rate are 8%, and tax-free customers are 0%. To change the rule, fix the registered details of the [customer](/manual/en/masters/customer/user).
