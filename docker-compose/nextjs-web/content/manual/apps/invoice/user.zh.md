...
本应用里没有「新規作成」（新建）按钮。发票是由[结算截止处理](/manual/zh/apps/billing-closing/user)自动做出来的。
...
- **由来（来源）** … 表示这一行是从哪张[出货单](/manual/zh/apps/shipping-order/user)或[送货单](/manual/zh/apps/delivery-note/user)来的。

## 开始之前

- 发票不能在本应用里做。请先在[结算截止处理](/manual/zh/apps/billing-closing/user)里点「請求書を生成」（生成发票）。
- 发行发票或推进记录，需要发票权限。
...
- **摘要（摘要）** … 产品名称和批次号。
- **由来（来源）** … 点蓝色文字，可以打开作为依据的[出货单](/manual/zh/apps/shipping-order/user)或[送货单](/manual/zh/apps/delivery-note/user)。
...
> ⚠️ 消费税栏里写的是「**消費税（10%）**」（消费税（10%）），但实际税额是按每个客户的规定（10% / 8% / 0%）计算的。请不要只看界面上的文字来判断税率。
...
点界面右上角的「**PDF**」，发票的 PDF 会在新标签页里打开，可以从那里打印或保存。
...
**Q. 找不到「新規作成」（新建）按钮。**
A. 发票不在这个界面做。请打开[结算截止处理](/manual/zh/apps/billing-closing/user)，在对应的行上点「請求書を生成」（生成发票）。

**Q. 想改金额，却没有编辑界面。**
A. 发票不能直接改。应该是作为依据的[出货单](/manual/zh/apps/shipping-order/user)的内容有问题，请联系财务或营业的负责人。
...
**Q. 消费税好像不是按 10% 算的。**
A. 消费税是按每个客户的规定计算的。适用轻减税率的客户是 8%，免税客户是 0%。想改规定时，请修改[客户](/manual/zh/masters/customer/user)的登记内容。
