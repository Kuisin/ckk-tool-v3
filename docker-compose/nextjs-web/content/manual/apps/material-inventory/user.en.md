---
title: "Material Inventory — User Manual"
description: "Operation code PD05. A ledger for checking material stock and reservations, plus \"when and how much will be available…"
screenshots: []
---
...
- **Scheduled receipts** … Line items of ordered [material purchase orders (素材発注書)](/manual/en/apps/purchase-order/user) (status 発注済) provide the next receipt date and incoming quantities.

## Reading the list

One row = material × plant.

- Columns: **material** (material code + name), **plant**, **quantity** (with unit), **reserved**, **available** (quantity − reserved), **next receipt** (earliest expected date among ordered line items), and **updated at**.
- Search by material code or name, and filter by **plant**.
- Click a row to open the detail screen.

## Detail screen

The summary shows the material, plant, quantity, reserved, available, next receipt, and storage location.
...
Using this app requires the inventory (inventory) permission. Product stock is checked in [Product Inventory](/manual/en/apps/product-inventory/user) (PD04).
