...
---
本页说明[库存管理](/manual/zh/apps/product-inventory/user)（操作码 `PD04`）的「**素材**」（材料）标签页和「**仕掛品**」（在制品）标签页的看法。可以确认材料现在能用多少，以及现在有多少正在制造中。
...
> 💡 材料数量无法在这里直接修改。数量会随着日常业务自动变动。本画面上能手动进行的操作只有「**在庫移動**」（库存移动，即更改存放位置）。步骤请参考[库存管理](/manual/zh/apps/product-inventory/user)。
...
- **增加** … 在[材料到货](/manual/zh/apps/material-receipt/user)（PU03）中登记到货后，收货据点的库存会增加。
- **被预留** … 「製造分」（制造分）的[制造指示书](/manual/zh/apps/work-order/user)获批后，要使用的材料会按计划数量被预留。
- **减少** … 该指示书的所有工序完成后，预留的材料会作为已使用的部分减少。
- **出现到货预定** … 已下单的[材料订购单](/manual/zh/apps/purchase-order/user)（PU02）的明细，会显示下次的到货预定日和数量。
...
- 点击指示书编号，会打开该[制造指示书](/manual/zh/apps/work-order/user)的画面。
- 没有进行中的指示书时，会显示「**進行中の仕掛品はありません**」（没有进行中的在制品）。

> ⚠️ 在制品还不是库存。只有指示书的所有工序完成后，才会进入实际库存。

使用本应用需要库存权限。产品标签页、位置标签页以及库存移动的步骤，请参考[库存管理](/manual/zh/apps/product-inventory/user)。
