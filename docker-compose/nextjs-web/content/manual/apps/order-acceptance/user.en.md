...
- If a unit price differs from the [price list](/manual/en/apps/price-list/user), **the screen tells you**.
...
- The customer must be registered in the [customer master](/manual/en/masters/customer/user). Without it, the customer stays 「未特定」(not identified) even after reading the form.
- The ordered [product](/manual/en/masters/product/user) must be registered in the product master.
- Having a [price list](/manual/en/apps/price-list/user) makes checking the unit price easier, but you can go on without one.
...
If you entered a quote number, that [quote](/manual/en/apps/quote/user) becomes 「受諾済」(accepted) automatically.
...
A. The company name on the order form could not be matched to the customer master. On the draft screen, search for the customer, choose it, and press「保存」(save). For customers whose name is often written in different ways, register the other spellings in the [customer master](/manual/en/masters/customer/user) and they will be matched automatically next time.
