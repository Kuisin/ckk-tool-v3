...
要让外协出现在这个列表中，请在[制造指示书](/manual/zh/apps/work-order/user)的工序中，把实施地点选为「**外注**」（外协），并指定外协厂。外协厂在[外协企业主数据](/manual/zh/masters/supplier/user)中登记。
...
- **指示書番号**（指示书编号）… 蓝色数字。点击会打开[制造指示书](/manual/zh/apps/work-order/user)画面。
- **製品**（产品）… 该加工所制造的产品名称。
...
无法从本应用添加。在[制造指示书](/manual/zh/apps/work-order/user)画面上，把工序的实施地点选为「**外注**」（外协），并指定外协厂，就会出现在这个列表中。
...
**Q. 列表是空的。**
A. 还没有包含外协工序的指示书。画面会显示「**外注工程がありません（指示書の工程で外注を選ぶと表示されます）**」（没有外协工序——在指示书的工序中选择外协后会显示）。在[制造指示书](/manual/zh/apps/work-order/user)的工序中把实施地点选为「外注」（外协），就会出现。
...
**Q. 材料的采购也在这里进行吗？**
A. 不是。购买材料请使用[材料订购单](/manual/zh/apps/purchase-order/user)。本应用只是委托外部公司做加工本身的一览。
