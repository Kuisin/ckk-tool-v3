...
---
A register where you record "this [product](/manual/en/masters/product/user) costs this much" for each customer. The operation code is `SA02`.

## What you can do with this app

- You can record the selling price of a product for each [customer](/manual/en/masters/customer/user).
- Once recorded, the amounts on a [quote](/manual/en/apps/quote/user) **fill in automatically** (you never have to type an amount on the quote screen).
- On an [order acceptance](/manual/en/apps/order-acceptance/user), the unit price on the customer's order form is **compared with the price here automatically**, and you are told if they differ.
...
- **基準単価 (base unit price)** … the basic price for that order type. Normally it comes from the result of a [trial estimate](/manual/en/apps/trial-estimate/user).
...
- The [customer](/manual/en/masters/customer/user) and the [product](/manual/en/masters/product/user) must already be registered.
- It helps to have a [trial estimate](/manual/en/apps/trial-estimate/user) to base the price on. If you set a product on the estimate and set it to 確定 (confirmed), you can choose it on this screen and the amount fills in automatically.
...
> ⚠️ If you see「この製品にリンクされた確定済みの試算はありません」(there is no confirmed estimate linked to this product), no estimate for that product has been confirmed yet. If you want to use an estimate, first set the product on the [trial estimate](/manual/en/apps/trial-estimate/user) and confirm it. If you do not want to use an estimate, tick「**カスタム単価を使用**」(use a custom price) and type the base unit price in by hand.
...
The [quote](/manual/en/apps/quote/user) creation screen opens with the contents already filled in. You can do the same from「**…**」on a row in the list.
...
A. A quote keeps the amounts from the time it was made. To use the new amount, open that [quote](/manual/en/apps/quote/user) and do「編集」(edit) →「保存」(save). The amounts are then filled in again from the price list as it is now.
