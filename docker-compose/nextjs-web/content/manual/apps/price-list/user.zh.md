...
- 基准单价通常从关联到产品的 **已确定试算（SA01）** 中取得（也可手动设置 — 需确认）。
...
2. 按订单类型设置价格。所选产品若有 **已关联的确定试算（SA01）**，可将其选为 **价格来源（试算）**，其报价单价会填入基准单价（推荐）。
