...
- 発注すると、その素材の[在庫画面](/manual/ja/apps/material-inventory/user)に「入荷予定」として出ます。
...
- 注文したい **素材が[素材マスタ](/manual/ja/masters/material/user)に登録されている** 必要があります。
- **仕入先が[外注企業マスタ](/manual/ja/masters/supplier/user)に登録されている** 必要があります。登録されていない会社は選べません。
...
[購買依頼](/manual/ja/apps/purchase-request/user)が承認されている場合は、その画面の「**発注書へ変換**」から作れます。素材・数量・拠点はそのまま引き継がれますが、**単価は 0 円のまま** です。下書きのうちに「編集」から単価を入れてください。
...
状態が「**承認依頼中**」に変わり、承認する人に依頼が届きます。同じ依頼は[承認管理](/manual/ja/apps/approval/user)の画面にも並びます。
...
状態が「**発注済**」になります。この時点で、注文した素材が[在庫画面](/manual/ja/apps/material-inventory/user)に「入荷予定」として表示されるようになります。
...
まだ届いていない残りの数量がまとめて「届いた」ことになり、入荷先拠点の素材在庫に加わります。同時に[素材入荷](/manual/ja/apps/material-receipt/user)の記録も自動で作られ、依頼した人と作った人にお知らせが届きます。

> ⚠️ 一部だけ先に届いた場合は、この操作を使わないでください。届いた分だけを[素材入荷](/manual/ja/apps/material-receipt/user)で登録してください。「入荷完了」は残り全部をまとめて処理します。
...
**Q.「承認」「差し戻し」のボタンが出ません。**
A. 承認できるのは、第一承認グループに入っている人（またはその代理の人）だけです。ボタンの代わりに「**第一承認グループのメンバーのみ承認・差し戻しできます**」と表示されます。
...
**Q. 素材が分けて届きました。**
A. 届いた分だけを[素材入荷](/manual/ja/apps/material-receipt/user)で登録してください。全部そろってから「入荷完了」を押すと、残りがまとめて処理されます。
