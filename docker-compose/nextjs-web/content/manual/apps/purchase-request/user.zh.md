...
- 已审批的申请，按一个按钮就会变成[材料订购单](/manual/zh/apps/purchase-order/user)。
- 当某种材料的库存变少、需要补货时使用。

## 本页出现的词语

- **明細**（明细）… 申请中的一行。写明「哪种材料、几根」。
- **素材**（材料）… 加工用的棒材等原料。只能从已登记在[材料主数据](/manual/zh/masters/material/user)中的材料里选择。
...
- 想申请的 **材料必须已登记在[材料主数据](/manual/zh/masters/material/user)中**。未登记的材料无法选择。
- 想指定收货地点时，该 **[据点](/manual/zh/masters/plant/user)必须已登记**。
...
4. **発注済**（已下单）… 已变成订购单。之后的操作在[材料订购单](/manual/zh/apps/purchase-order/user)中进行。
...
系统会创建一份沿用相同明细的[材料订购单](/manual/zh/apps/purchase-order/user)（草稿），并跳转到该画面。申请一侧会变成「**発注済**」（已下单），并显示指向新订购单编号的链接。
...
**Q. 找不到填写价格的栏位。**
A. 采购申请没有价格栏。价格在转换为[材料订购单](/manual/zh/apps/purchase-order/user)之后才输入。
