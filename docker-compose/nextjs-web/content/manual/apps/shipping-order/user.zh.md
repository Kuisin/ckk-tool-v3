...
- 记录发货以后，**库存会自动减少**。
- 可以从出货单做出[送货单](/manual/zh/apps/delivery-note/user)。
- 不发出去、留在公司里保管的部分（比如多做的备品）也可以记录。

出货单是很重要的单据，以后做[发票](/manual/zh/apps/invoice/user)时就以它为依据。
...
- 请确认产品已经做好（[指示書](/manual/zh/apps/work-order/user)已经完成）。完成的指示书的量会自动填进明细。
...
发货后，当天的日期会记为 **出荷日（发货日）**，状态变成「**出荷済**」（已发货）。同时，发出的数量会从[产品库存](/manual/zh/apps/product-inventory/user)里减掉，销售订单那边也会自动变成「一部出荷」（部分发货）或「出荷済」（已发货）。
...
2. 点「**納品書を作成**」（创建送货单）。
3. 会打开[送货单](/manual/zh/apps/delivery-note/user)的创建界面，出货单已经事先选好。
...
A. 发货据点里没有足够的库存。请在[产品库存](/manual/zh/apps/product-inventory/user)里确认数量后再试一次。
