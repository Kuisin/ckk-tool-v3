...
- 把保存的试算设为「確定」（确定）后，就可以作为制作[价格表](/manual/zh/apps/price-list/user)时的价格来源。
...
- 如果打算把计算结果用在[价格表](/manual/zh/apps/price-list/user)里，请先登记好[产品](/manual/zh/masters/product/user)。试算里没有指定产品的话，在价格表中就选不到。
...
- **「試算番号」（试算编号）** … 以 `EST-` 开头的编号，保存后会自动编上。
...
12. 最后在「**試算名**」（试算名称）里输入名称（必填）。
13. 点击画面右上角的「**保存**」（保存）。

![试算新建表单](../../assets/screenshots/trial-estimate-new-01.png)

每输入一项，画面下方的「**試算結果**」（试算结果）都会当场重新计算。
...
> 💡 数量多时的折扣（让每支更便宜的设定）不在这个画面设置，而是在[价格表](/manual/zh/apps/price-list/user)里设置。试算只给出每支的基准价格。
...
确定后状态会变成「**確定**」（确定），画面上方会出现「確定済み」（已确定）的提示。这样在制作[价格表](/manual/zh/apps/price-list/user)时就能选到这份试算了。
...
1. 打开作为基础的试算。
2. 从右上角的「**…**」中点击「**複製して再試算**」（复制后重新试算）。
3. 会打开内容相同的新输入画面，只改想改的地方。
4. 点击「**保存**」（保存）。

试算名称会自动加上「（再試算）」（重新试算），编号会编上新的号码。在列表某一行的「**…**」中也能做同样的操作。

## 常见问题与困扰

**Q. 保存后的试算不能编辑。**
A. 试算的机制是把保存当时的价格作为记录保留下来，所以之后不能修改。请用「**複製して再試算**」（复制后重新试算）重新做一份。

**Q. 出现「試算名を入力してください」（请输入试算名称），无法保存。**
A. 试算名称是必填项目。请在画面最下方的「試算名」（试算名称）里输入一个以后好找的名字。

**Q. 制作价格表时选不到这份试算。**
A. 价格表里能选的只有 **「指定了产品，并且状态为確定（确定）」的试算**。请在试算画面的「**…**」中，依次操作「製品にリンク」（关联到产品）→「確定」（确定）。

**Q. 出现「下書きの試算のみ確定できます」（只有草稿试算才能确定）。**
A. 该试算已经确定，或已经用于价格表，不需要再确定一次。

**Q. 出现「価格表で使用済みの試算は製品リンクを変更できません」（已用于价格表的试算不能更改产品关联）。**
A. 已经用于价格表的试算（状态为「価格表登録済」）不能修改内容。想用别的产品计算时，请用「複製して再試算」（复制后重新试算）重新做一份。
