...
- 首先需要有 **注文請書**（销售订单）。销售订单由[订单受理](/manual/zh/apps/order-acceptance/user)的导入画面生成，可以在「注文請書」（销售订单）画面（操作码 `PD01`）中确认。
- 在销售订单画面执行「**在庫照合**」（库存核对）后，已有的库存会被为该订单预留。不足的部分用本应用来制造。
- 负责审批的人必须事先登记在[审批组](/manual/zh/masters/approval-group/user)中。
...
也可以从[审批管理](/manual/zh/apps/approval/user)（PD03）的列表中进行审批。
...
- **不良内訳**（不良明细）… 只在出现不良品时输入。点击「**不良を追加**」（添加不良），每一行选择以下内容：
  ...
  - 原因 … 从「**不良種類を選択**」（选择不良种类）中选择（在[不良种类](/manual/zh/masters/defect-type/user)中登记的内容）
  - 根数
- 最后点击「**工程完了**」（工序完成）。
...
库存可以在[库存管理](/manual/zh/apps/product-inventory/user)（PD04）中确认。正在制造中的根数可以在同一应用的「**仕掛品**」（在制品）标签页中查看。
