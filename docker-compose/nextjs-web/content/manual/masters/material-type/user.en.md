...
Here you register "whose material it is and what kind it is". **If a material type is not registered, you cannot set the material for a [Product](/manual/en/masters/product/user) or a [Material](/manual/en/masters/material/user).** You also cannot work out the material cost in a [Trial Estimate](/manual/en/apps/trial-estimate/user).
...
To register a material type, the **maker, the maker grade and the shape** must already be registered in [Numbering Setup](/manual/en/masters/material-numbering/user). If the maker you want does not appear, please register it there first.
...
> 💡 Some material types brought over from the old system do not have a code yet. Those rows show 「**未変換**」 (Not converted) in gray. You can correct the name and so on, but **you cannot choose them when you make a [Material](/manual/en/masters/material/user).** If you choose 「未変換」 (Not converted) in the 「**変換状態**」 (Conversion status) field, only those material types are shown.
...
When there is no purchase record for the material yet, the material cost in a [Trial Estimate](/manual/en/apps/trial-estimate/user) uses the price you register here. **If this is empty, the trial estimate cannot show a material cost.**
