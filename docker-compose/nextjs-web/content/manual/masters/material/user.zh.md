...
如果说[材种](/manual/zh/masters/material-type/user)表示「是什么样的材料」，那么「素材」（材料）就是 **「该材种中，这个直径、这个长度、这种表面处理的那一根」**，也就是实际买回来放在货架上的那一根。没有在这里登记的材料，在[材料订购单](/manual/zh/apps/purchase-order/user)和[库存管理（材料・在制品）](/manual/zh/apps/material-inventory/user)中 **无法选择**。
...
- **素材（材料）** … 实际采购并库存的材料棒。
- **材種（材种）** … 材料的种类，相当于材料的「上级」（在[材种](/manual/zh/masters/material-type/user)中登记）。
...
要登记材料，必须先 **登记好[材种](/manual/zh/masters/material-type/user)**。材料一定隶属于某一个材种。

另外，使用本应用需要 **主数据权限**。画面打不开时，请咨询管理员。

另请注意，[产品](/manual/zh/masters/product/user)是以「材种 + 直径 + 全长」来指定材料的。**产品并不会与某个特定的材料绑定。** 本台账只用于采购和库存。
...
A. 所选材种还没有代码。请在[材种](/manual/zh/masters/material-type/user)画面重新登记后，再选择那个材种。

**Q. 在材种栏中输入后，找不到想用的材种。**
A. 只有带代码的材种才能选择。栏位下方也会显示「変換済（コード構成あり）の材種のみ選択できます」（仅可选择已转换・具有代码结构的材种）。请在[材种](/manual/zh/masters/material-type/user)画面重新登记。
