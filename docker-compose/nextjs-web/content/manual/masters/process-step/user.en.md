...
The steps on a [指示書 (work order)](/manual/en/apps/work-order/user) are chosen from the ones registered here and lined up in order. Work that is not registered here cannot be chosen on a work order.
...
A. An [inspection sheet template (検査表テンプレート)](/manual/en/masters/inspection-template/user) has this step set as its related step. Do not delete it; deactivate it instead.
