...
[指示書](/manual/ja/apps/work-order/user) の工程は、ここに登録したものの中から選んで並べます。ここに無い作業は、指示書でも選べません。
...
7. 必要なら「**既定作業時間**」に、ふつうこのくらいかかるという時間を入れます（単位は時間です）。
8. 「**表示順**」に数字を入れます。小さい数字ほど一覧の上に出ます。
9. 「**保存**」を押します。
...
A. [検査表テンプレート](/manual/ja/masters/inspection-template/user)がこの工程を「関連工程」に指定しています。消さずに「無効化」してください。
...
**Q. 表示順を変えたのに、指示書の工程の順番が変わりません。**
A. 表示順は一覧の見た目だけの順番です。実際に作業する順番は、「実行依存」で決めた決まりから決まります。
