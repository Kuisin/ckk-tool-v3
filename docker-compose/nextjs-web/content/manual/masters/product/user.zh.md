...
**如果没有登记产品，就无法制作[试算](/manual/zh/apps/trial-estimate/user)、[价格表](/manual/zh/apps/price-list/user)和[报价单](/manual/zh/apps/quote/user)。** 有新产品的业务时，请先在本画面登记。
...
- **材種（材种）** … 材料的种类，表示是哪家厂商的、什么样的材料（在[材种](/manual/zh/masters/material-type/user)中登记）。
...
要登记产品，必须先 **登记好[材种](/manual/zh/masters/material-type/user)**。材种用于选择「使用什么材料」的栏位。

不选材种也可以登记产品，但之后做[试算](/manual/zh/apps/trial-estimate/user)时就算不出材料费。建议尽量把材种也一并填写。
...
> 💡 可选择的项目和类别由管理员设定。没有需要的项目时，请咨询[产品种别](/manual/zh/apps/product-type/settings)的负责人。
...
在「工程」（工序）标签中，可以登记制作该产品时的工序顺序。登记好之后，在制作[制造指示书](/manual/zh/apps/work-order/user)时工序会 **预先填好**，不必每次都从零开始选择工序。
...
**Q. 在材种栏中输入后，找不到想用的材种。**
A. 只有「已登记代码结构」的材种才能选择。栏位下方也会显示「変換済（コード構成あり）の材種のみ選択できます」（仅可选择已转换・具有代码结构的材种）。从旧系统导入的材种无法选择，请在[材种](/manual/zh/masters/material-type/user)中重新登记。
