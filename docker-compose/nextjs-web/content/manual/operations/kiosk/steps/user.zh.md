...
| [良品数 / 合格数](#field-success) | 必填 | 可交给下一工序的数量 |
| [不良明细](#field-defects) | 条件必填 | 半成品・废弃・工序分支的数量 |
| [不良理由](#field-reasons) | 选填 | 各不良种类的数量 |
...
必须满足**接收数 = 良品数 + 不良合计**。不一致时画面会显示警告。

### 不良理由 [#field-reasons]

各不良种类的数量。**不影响库存** — 是日后回顾原因用的辅助记录。
