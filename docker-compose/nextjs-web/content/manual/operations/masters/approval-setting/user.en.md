...
- Every step carries a badge with the number of people who can approve it right now. Hover the badge to see their names.
  - **N can approve** (green) … that many people can approve the step.
  - **No members** (red) … nobody can approve that step at the moment, so a request would stall there.
- A card with a zero-member step is bordered in red, the same as an unconfigured one.
- **Only members of the step's approval group (and stand-ins inside their period) can approve.** No separate permission is needed, and no role makes a non-member able to approve.
...
Once a group is chosen, the count and the member names appear below it as **people who can approve this step** — so you can check before saving that someone can actually press the button.
...
Above the member table you can see **which documents this group approves** and the steps it covers. A group that is not used in any approval flow says so (adding members to it does nothing).
...
> 💡 The people listed here whose state is **Permanent** or **Active** are exactly the people who can approve. Roles and permissions in User management (SY01) play no part.
...
A. Only a **member of that step's group** (or a stand-in inside their period) whose state is **Permanent** or **Active** can approve. Check the step's group on the Approval flows tab, then check on that group's Members tab whether the person is there and what state they are in (**Disabled**, **Scheduled** and **Ended** cannot approve). Adding roles or permissions will not help.

**Q. A step says "No members".**
A. Nobody can approve that step right now, so a request would stall there. Add people on that group's Members tab, or point the step at a different group.
