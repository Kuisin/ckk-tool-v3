...
ここで登録した分類が、[指示書](/manual/ja/operations/production/work-order/user)の工程で不良を記録するときの選択肢になります。不良は 1 行ごとに「区分・不良種類・詳細・本数」で記録し、**不良種類は必ず選びます**。すべての行がそろうまで工程を完了できず、不良数の集計もこの記録から自動で計算されます。
...
複数の行にチェックを入れて、「**一括無効化**」でまとめて外すこともできます。もう一度使いたくなったら、同じ手順で「**有効化**」を選びます。チェックした行をまとめて消す「**一括削除**」もありますが、不良の記録が残っている分類は消せません。
...
不良種類の登録画面で入力する項目の一覧です。

| 項目 | 何を入れるか |
|------|-------------|
| [コード / 名称](#field-code) | 不良種類の管理番号と名前です |
| [表示順](#field-sort-order) | 選択肢での並び順です |
| [有効](#field-active) | 外すと、不良記録の選択肢に出なくなります |
...
外すと、不良記録の選択肢に出なくなります。**有効な分類が 1 つも無いと、現場で不良を記録できなくなります**。少なくとも 1 つは有効な分類を残しておいてください。
...
**Q. 「不良記録で使用中の不良種類は削除できません（無効化してください）」と出て消せません。**
A. その分類を使った不良の記録が残っています。記録を守るための仕組みなので、削除ではなく「無効化」してください。
