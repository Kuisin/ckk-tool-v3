...
这里登记的分类，会成为在[指示书](/manual/zh/operations/production/work-order/user)的工序中记录不良时的选项。
...
也可以勾选多行，用「**一括無効化**」（批量停用）一起去掉。想重新使用时，按同样的步骤选择「**有効化**」（启用）。
...
**Q. 出现「不良種類の削除に失敗しました」（不良种类删除失败），无法删除。**
A. 很可能还留有使用了该分类的不良记录。这是为了保护记录的机制，请不要删除，改为「停用」。
