...
- **订单请书**（订单请书）… 虽然会出现在列表中，但由于还没有专用的审批画面，点击该行不会跳转
...
- **種別**（类别）… 用带颜色的徽章显示是「指示書」（制造指示书）「素材発注書」（材料订购单）「購買依頼」（采购申请）「订单请书」（订单请书）中的哪一种。
- **対象番号**（对象编号）… 该单据的编号。
...
打开本列表需要「审批管理」的查看权限。能否实际按下审批・退回，并不由权限决定，而取决于是否属于该单据的 **审批组**（MS0B 审批设定）。
...
**Q. 点击某一行也没有任何反应。**
A. 那是类别为「订单请书」（订单请书）的行。该单据还没有专用的审批画面，因此只是显示在列表中。
