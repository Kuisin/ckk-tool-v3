...
- **注文明細（ちゅうもんうけしょ）** … お客様の注文を、製品ごと・本数ごとに分けた社内の書類です。指示書はこれを選んでつくります。
...
- 先に **注文明細** が必要です。注文明細は[注文請書](/manual/ja/operations/sales/order-acceptance/user)の取込画面から作られ、「注文明細」の画面（操作コード `PD01`）で確認できます。
...
2. 「**注文明細**」の欄をクリックして、もとになる注文明細を選びます。注文明細番号・製品・顧客で検索できます。
...
- 注文明細を選ぶと、その製品の工程リストが自動で選ばれ、工程が入った状態になります。
...
2. 状態が「**承認待ち**」に変わります。ここから先、もとの注文明細は編集できなくなります。
...
| 項目 | 必須 | 何を入れるか |
|------|------|-------------|
| [注文明細](#field-order-line) | 任意 | どの受注に対する指示書か |
