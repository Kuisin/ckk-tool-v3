...
5. 「**種別**」で「在庫分」か「製造分」を選びます。在庫分は明細を 1 つだけ選べます。**在庫分の工程は「製品出し（在庫）」＋（必要なら）「出荷前検査」の固定構成**で、工程リストは使いません — 以降の 10〜11 は製造分だけの手順です。
...
- 一番上の「**出し・受渡し（開始）**」（素材出し・半製品出し・素材受渡し・製品受渡し）から**必ず 1 つ以上**選びます。**全ての工程はここから始まります。**
