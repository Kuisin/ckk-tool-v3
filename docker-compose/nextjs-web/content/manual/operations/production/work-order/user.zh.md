...
- **指示書番号**（指示书编号）… 类似 `WO-202608-00001` 的编号，与其他单据（报价单、订单回执等）格式相同，每月从 1 重新编号。批次号（类似 `#9001` 的流水号）另行编号，可在详情页面确认。
...
5. 在「**種別**」（类别）中选择「在庫分」（库存分）或「製造分」（制造分）。库存分只能选择 1 条明细。
...
9. 「**検査表**」（检查表）会自动选好需要的项目。不足时可以再追加。
10. 选择「**工程リスト**」（工序列表）（见下一小节）。
