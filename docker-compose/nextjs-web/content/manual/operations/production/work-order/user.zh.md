...
- 首先需要有 **订单明细**（订单明细）。订单明细由[订单请书](/manual/zh/operations/sales/order-acceptance/user)的导入画面生成，可以在「订单明细」画面（操作码 `PD01`）中确认。
...
2. 点击「**订单明细**」（订单明细）栏，选择作为依据的订单明细。可以按订单明细编号、产品、客户搜索。
...
| 项目 | 必填 | 填什么 |
|------|------|--------|
| [订单明细](#field-order-line) | 选填 | 针对哪笔受订的指示书 |
