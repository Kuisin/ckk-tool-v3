...
5. 在「**種別**」（类别）中选择「在庫分」（库存分）或「製造分」（制造分）。库存分只能选择 1 条明细。**库存分的工序是固定构成 —「製品出し（在庫）」（产品出库）加上（需要时）「出荷前検査」（发货前检查）— 不使用工序列表**，因此下面第 10〜11 步只适用于制造分。
...
11. 既可在公司内做也可外协的工序，请选择「**社内**」（公司内）或「**外注**」（外协）；公司内要选据点，外协要选外协厂。
12. 点击「**保存**」（保存）。
...
- 选择订单明细后，该产品的工序列表会被自动选中，工序也会随之填入。
...
3. 负责第 1 级审批的人点击「**承認**」（审批）。
4. 如果审批设置中还有第 2 级以后的级别，则由各级别的人依次点击「**承認**」。通过最后一级后即为已审批。
5. 两个阶段都完成后，状态会变成「**承認済**」（已审批），就可以开始作业了。

![制造指示书的审批状况面板](../../../assets/screenshots/work-order-approval-01.png)

- 只有属于该级别 **审批组的人**，以及按期间被指定的代理人才能审批。不属于时不会出现按钮，而会显示「◯◯ のメンバーのみ承認・差し戻しできます」（仅该组成员可以批准或退回）。
...
制造的数量。请输入**不低于分配合计**的数（可自由加上不良备用量；库存分与分配合计相同）。也会作为**工序接收数量的初始值**。
