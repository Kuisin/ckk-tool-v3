...
- 用「受注時」（接单时）登记时，先有注文請書（销售订单）就能关联起来（销售订单由[订单受理](/manual/zh/operations/sales/order-acceptance/user)的「伝票展開」（单据展开）生成）。
...
| [订单请书](#field-sales-order) | 条件必填 | 受订后选择的对应订单请书 |
| [产品](#field-product) | 选填 | 需要制图的产品 |
| [委托内容](#field-description) | 必填 | 希望设计什么 |

### 触发时机 [#field-trigger]

该委托是 **为了报价** 还是 **受订确定之后** 提出的。选择不同，接下来关联的是「报价单」还是「订单请书」。
