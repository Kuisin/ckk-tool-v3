...
4. 画面の右上に進み具合が表示されます。読み取りは 1 件あたり **30 〜 60 秒ほど** かかります。
...
4. 届け先や担当が決まっていれば「**出荷先**」「**担当拠点**」「**出荷作業場所**」も選びます（どれも任意です。あとから入れられます）。
5. 明細の行で「**製品**」「**種別**」「**数量**」を入れます（単価はあとから入れても構いません）。
6. 行を増やしたいときは「**明細を追加**」を押します。
7. 「**下書きを作成**」を押します。
...
6. 必要に応じて「**出荷先**」「**担当拠点**」「**出荷作業場所**」を選びます（任意です）。
7. 「**明細**」で、製品・種別・数量・単価・納期を確認します。
8. オレンジの「**製品未特定**」バッジが付いている行は、「**製品**」の欄で正しい製品を選びます。欄の下に「**もしかして**」として候補が出ていれば、押すだけで入ります。
9. 画面のいちばん下の「**保存**」を押します。保存すると閲覧に戻ります。
