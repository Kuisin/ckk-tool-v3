...
2. 需要修改时点击「**編集**」（编辑）——在画面右上，或最上方的卡片中。画面会变成可输入的表单。
...
1. 在查看模式下，点击画面最上方卡片中的「**承認依頼**」（申请审批）。（编辑中不会显示该按钮，请先保存。）
...
**Q. 草稿的内容改不了。**
A. 草稿画面以只读方式打开，请先点击「**編集**」（编辑）——在画面右上，或最上方的卡片中。另外，只有状态为「下書き」（草稿）时才能修改内容。审批中、已审批、已展开时都不能修改。出现「下書きの订单请书のみ編集できます」（只有草稿状态的订单请书才能编辑）时，请让人退回。
