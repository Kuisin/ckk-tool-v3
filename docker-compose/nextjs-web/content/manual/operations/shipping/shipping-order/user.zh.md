...
- **受注明細（销售订单）** … 写明「发给哪个客户、什么产品、多少支、什么时候交」的单据。做出货单时要看它。
...
1. 点一览界面右上角的「**新規作成**」（新建）。
2. 点「**受注明細**」（销售订单）栏，选要发货的销售订单。在这个栏位里，是用 **客户名称、产品名称、客户的订单编号** 来查找的（与一览界面的搜索框不同，这里无法用 `ORD-` 开头的销售订单号查找）。
...
| 项目 | 必填 | 填什么 |
|------|------|--------|
| [受订明细](#field-order-line) | 必填 | 针对哪笔受订发货 |
...
**Q. 在新建界面的「受注明細」（销售订单）栏里，搜索不到想发货的销售订单。**
