...
A. The switch only decides whether the app appears on screen at all. Separately from that, the person also needs permission to use that app. Please check that person's permissions in [User Management](/manual/en/operations/system/user-management/user).
...
A. You can check in the [Activity Log](/manual/en/operations/system/activity-log/user). Narrow 「**対象**」 (Target) to 「**アプリ管理**」 (App Management) and only the switch changes are listed.
