...
- 「**メンテナンス PIN（全端末共通・毎日 4:00 自動更新）**」… タブレットの画面を一時的に自由に使えるようにするための番号です。Wi-Fi の設定を変えるときなどに使います。端末画面の左上を 5 回続けてタップすると入力欄が出ます。
- 「**端末設定コード（この端末・左上 5 タップ用）**」… そのタブレット 1 台だけの番号です。端末をリセットしたり結び付け直したりするときに使います。「**再生成**」を押すと新しい番号になり、**前の番号は使えなくなります**。
