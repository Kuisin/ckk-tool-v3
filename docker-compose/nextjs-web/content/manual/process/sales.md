...
| 4. 注文を受ける | お客様から届いた注文書を取り込み、内容を確認して受諾します | 社内営業補助 | [受注請書](/manual/ja/operations/sales/order-acceptance/user)（`SA04`） |
| 5. 受注を確定する | 受諾した内容から注文請書が作られます（製品ごと） | 社内営業補助 | 注文請書（`PD01`） |
| 6. 製造を指示する | 在庫分と製造分に分け、工程を並べた指示書を作ります | 社内営業補助 | [指示書](/manual/ja/operations/production/work-order/user)（`PD02`） |
...
### 5〜6. 受注の確定と製造の指示（注文請書・指示書）

受注請書を確定すると、明細ごとに **注文請書** が作られます。続いて、その注文請書に対して **指示書** を作ります。指示書では在庫から出す分と新しく作る分を分け、工程を順番に並べます。ここから先は生産の流れになります。
...
| 受注請書 | 取込中 → 下書き → 承認依頼中 → 承認済 → 展開済 → アーカイブ |
| 注文請書 | 下書き → 確定 → 製造中 → 一部出荷 → 出荷済 |
...
**注文の価格が見積と違う**
受注請書の画面で差異が分かります。見積書に戻って価格を調整してから、受注請書の内容を直してください。

**受注請書が次に進まない**
承認依頼が済んでいるか、承認者が承認したかを確認してください。承認済みになると注文請書・指示書へ展開できます。
