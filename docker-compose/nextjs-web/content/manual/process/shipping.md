...
| 段階 | 何をするか | 担当 | 使うアプリ |
|------|-----------|------|-----------|
| 1. 出荷書を作る | 注文請書を選び、出す明細とロットを決めます | 出荷担当 | [出荷書](/manual/ja/operations/shipping/delivery-order/user)（`SH01`） |
...
**注文請書**を選ぶと、出荷できる注文明細が明細ごとのまとまりで入り、数量は指示書の完成数から自動で入ります。出す**ロット**はまとまりごとに選べます。ロットは指示書の番号と同じもので、どの製造分を出したかがこれで残ります。出荷元の拠点も選びます（**その拠点の在庫が減ります**）。受注の残りを超える出荷はできず、残りに満たないときは「一部出荷」の確認が出ます。
