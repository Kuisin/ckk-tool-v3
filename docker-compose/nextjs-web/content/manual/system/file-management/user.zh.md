...
- **系统文件** … 不是人上传的，而是 **系统自动生成的文件**。报价单、发票的 PDF 就属于这一类。
...
> 💡 如果报价单或发票的 PDF 没有出现，请打开「**システムファイル**」（系统文件）开关。系统生成的文件平时是隐藏的。
...
**Q. 找不到本该存在的文件。**
A. 有两种可能。一是因为它是系统生成的文件而被隐藏了，请试着打开「**システムファイル**」（系统文件）开关。二是您没有查看该文件夹的许可，请与系统管理员联系。
