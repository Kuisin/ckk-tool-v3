...
- The tablets themselves are registered in [Device Management](/manual/en/system/kiosk-device/user), and the cards used to log in are made in [QR Card Management](/manual/en/system/kiosk-card/user).
...
A. Please check two things. First, whether the switch on this screen is on. Second, whether that person has permission to use that app (you can check in [User Management](/manual/en/system/user-management/user)). The app appears only when both are true.
