...
- 「**アクション**」（操作）… 表示可以做什么。`READ` 是只能查看，`CREATE` 是新建，`UPDATE` 是修改，`DELETE` 是删除，`ADMIN` 是作为管理员的操作。
- 「**スコープ**」（范围）… 表示这个许可能到达多远。`ALL` 是全公司，`PLANT` 是只限自己的据点，`REGION` 是只限自己的地区，`OWN` 是只限自己创建的数据。
...
A. 可以在 [操作历史](/manual/zh/system/activity-log/user) 中确认。所有变更都有记录。
