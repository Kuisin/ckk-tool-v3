...
Create the **delivery note (納品書)** that accompanies shipped products. When you pick a [shipping order](/docs/apps/shipping-order/user) (confirmed or shipped), the recipient and line items are filled in automatically.

- The recipient is determined automatically from the shipping order → sales order's [customer](/docs/masters/customer/user) (plus branch) and cannot be changed.
...
- **Direct to end user (ユーザー直送)**: the shipment goes straight to the [end user](/docs/masters/end-user/user). Selecting the **destination (end user)** is required, and price printing defaults to "off".
...
- The delivery note number appears as a linked "source" on [invoice](/docs/apps/invoice/user) line items (the invoice itself is generated from shipping orders by the [billing closing](/docs/apps/billing-closing/user)).
