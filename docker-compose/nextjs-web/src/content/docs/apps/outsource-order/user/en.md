...
A list of only those steps of [work orders (指示書, PD02)](/docs/apps/work-order/user) whose execution location is **outsourced**. You can see at a glance which processing is currently out at which subcontractor and when it is expected back.
...
You cannot add one from this app. Choose **outsource** as the execution location of a work order step and assign a subcontractor — the step then appears in this list. Subcontractor companies are managed in the [supplier master](/docs/masters/supplier/user).
...
**Do I order materials here?** — No. Material ordering is done with the [material purchase order (素材発注書, PU03)](/docs/apps/purchase-order/user). This app lists outsourced manufacturing steps only.
