# Defect Types — User Manual

Operation code **MS09**. A small master for registering the categories used when recording defects during manufacturing (scratch, chip, dimensional defect, etc.). The types registered here become the choices in the defect record on the step execution screen of a [work order (指示書)](/docs/apps/work-order/user).
