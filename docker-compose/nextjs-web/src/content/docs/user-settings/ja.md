...
- **プッシュ通知**：「設定 → 通知設定」でデバイスごとに有効化すると、ロック画面・デスクトップに通知が届きます。

## プッシュ通知の有効化（デバイス別）

- **Chrome / Edge（パソコン）**：「設定 → 通知設定」→「このデバイスで有効化」を押し、ブラウザの通知許可で「許可」を選びます。
...
- 「設定 → 通知設定」の「登録デバイス」で、プッシュ通知を有効にしたデバイスの一覧を確認・削除できます。使わなくなった端末は削除してください。
