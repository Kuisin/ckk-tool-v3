-- allow-destructive: invoices.yayoi_exported_at の RENAME。ローリングデプロイ中は
-- 旧コンテナがまだ旧列名で書こうとするが、(1) この列に書くのは
-- app/api/export/accounting の 1 ルートだけ、(2) 刻印を取り逃しても
-- `force=1` で出し直せば付け直せる（データは失われない）ので、
-- 短い窓を許容する（利用者判断）。expand/contract にすると
-- 二重書きの期間ぶんコードが残り、そちらのほうが消し忘れやすい。
