...
--   work_orders.material_id                    指示書が使う素材
...
ALTER TABLE "app"."work_orders"                   ADD COLUMN "item_id" INTEGER;
...
CREATE INDEX "work_orders_item_id_idx"                   ON "app"."work_orders"("item_id");
...
ALTER TABLE "app"."work_orders"                   ADD CONSTRAINT "work_orders_item_id_fkey"                   FOREIGN KEY ("item_id") REFERENCES "app"."items"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
BEGIN
  ...
  + (SELECT count(*) FROM app.work_orders                   WHERE material_id IS NOT NULL AND item_id IS NULL)
  INTO n;
  IF n > 0 THEN
