...
-- 承認（承認・差し戻し）はこのファイルの管轄ではない。可否を決めるのは
-- 承認管理（承認フロー approval_flow_steps + 承認グループ approval_group_members）
-- だけで、RBAC の付与は関係しない。かつて存在した APPROVE 付与は廃止した
-- （rbac-seed.sql が残存行を毎回削除する）。
--
...
--   manager    : 全業務 R+E（横断閲覧。承認できるかは承認グループが決める）
...
-- manager: 全業務コード（system 以外）に R + E
-- （承認そのものは権限で決まらない — 承認グループへ入れること）
...
-- quality（検査記録・検査承認はどちらも work_order の UPDATE。work_order は拠点スコープ）
INSERT INTO app.role_permission_relation (role_id, permission_code, action, scope)
SELECT r.id, g.code, g.action::app."ACTION",
