-- ⚠️ **現行スキーマでは動かない（2026-08-25 時点）。参照用に残しているだけ。**
--    DB squash 以前の列（`e.base_unit_price` 等 — 今は price_list_variants 側）を
--    前提にしているため。imports/ への生成は make_imports.sh で止めてある
--    （置いたままだと `pnpm import:legacy` が総なめして復旧手順ごと落ちる）。
--    使うときは現行スキーマに合わせて書き直すこと。
--
