...
const EXCLUDED = [
  ...
  // 会計連携（仕訳 CSV）の**既定の列見出し**（伝票日付/借方科目コード/摘要…）。
  // 会計ソフトの受入フォーマットが期待する固定の日本語語彙で、CSV ヘッダーと
  // 同種の外部フォーマット契約——UI 語彙ではないので言語ごとに変えると取り込め
  // なくなる。しかも**設定の既定値**であって画面に出る文言ではない（見出しは
  // SY0J 会計連携で環境ごとに書き換える）。
  /\/accounting-export-core\.ts$/,
